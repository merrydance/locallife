...
assert(
  packagingPageTs.includes('const savedForm = await savePackagingDraft(currentForm)') &&
    packagingPageTs.includes('refreshErrorMessage: \'包装设置已保存，但最新状态同步失败，请稍后重新进入确认\''),
  'packaging page should keep the saved draft snapshot and surface a safe re-entry state when reload fails'
)

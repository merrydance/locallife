...
function main() {
  ...
  assert(!bankFormTs.includes('this.emitDraftChange(initialDraft)'), 'bank form must not emit draftchange during attached initialization')
  ...
  assert(submitFormSource.includes('opening-mode-choice') && submitFormSource.includes('bindtap="onOpeningModeChange"'), 'opening mode selector should use plain tap choices')
  assert(submitFormSource.includes('<view hidden="{{accountOpeningMode !== \'business\'}}">'), 'business form should stay mounted and use hidden to avoid custom component branch recreation')
  assert(submitFormSource.includes('<view hidden="{{accountOpeningMode !== \'personal\'}}">'), 'personal form should stay mounted and use hidden to avoid custom component branch recreation')
  assert(!submitFormSource.includes('<applyment-bank-form\n        wx:if="{{accountOpeningMode === \'business\'}}"'), 'business form must not be recreated through wx:if on opening mode changes')
  assert(!submitFormSource.includes('<baofu-personal-profile-form\n        wx:else'), 'personal form must not be recreated through wx:else on opening mode changes')
  ...
  assert(!submitFormSource.includes('<t-check-tag') || !submitFormSource.includes('bind:change="onOpeningModeChange"'), 'opening mode selector must not use TDesign check-tag because it crashes in current Mini Program runtime')

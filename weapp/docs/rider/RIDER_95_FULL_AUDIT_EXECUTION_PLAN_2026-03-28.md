...
本计划用于把微信小程序骑手侧从“主入口能看到部分数据，但深链、积分、异常、资金和孤岛页都存在真实接口漂移”的状态，提升到稳定 95 分以上。
...
3. 高值单资格积分页整页基于错误 DTO 渲染，当前不具备真实可用性。
...
当前 rider 子包注册页面共 8 个，以 app.json 为准：
...
2. 次链：deposit/index、exception/index。
3. 孤岛页：claims/index、credit/index、delivery/manage/manage。
...
3. exception、claim、appeal、recovery 各自页面职责清晰，不再混用。
...
3. credit/index 整页 DTO 错位。
4. exception/index 和 claims/index 领域混用。
5. delivery/manage/manage 旧状态机与错误路径仍在注册页内。

### 5.2 P1 领域能力缺口

1. deposit/index 未接提现能力。
2. exception/index 未接延迟申报和真实异常历史。
3. claims/index 未闭环追偿支付。
4. dashboard/index 缺稳定历史、信用分、异常入口收口。
...
目标：把 credit、deposit、exception 从“半成品页面”变成职责明确的骑手工具面。

二级任务：

1. credit/index 按真实 premium score 合同重建展示模型。
2. deposit/index 补提现入口、失败回查和资金状态。
3. exception/index 只保留真实异常上报职责，并补 delay / history 策略。
4. 如果需要展示 claim / appeal / recovery，则拆成独立页面，不再混入异常页。

退出标准：

1. 资金、积分、异常三域各自有唯一职责页面。

### Phase 3：孤岛页治理与历史实现收口

目标：处理 claims/index、delivery/manage/manage 等历史污染面，避免继续误导开发和测试。

二级任务：

1. claims/index 明确是保留为“索赔与申诉页”还是下线并拆分。
2. delivery/manage/manage 明确迁移有效能力或直接下线路由。
3. 清理错误的 service、旧状态枚举、未绑定交互和死导航。
4. 明确 credit/index 是否需要正式入口；若需要，补入口；若不需要，收口注册状态。
...
1. 对 dashboard、task-detail、tasks、deposit、exception 做五态验证。
2. 对抢单、取餐、送达、异常上报、充值、提现、追偿支付做主链验证。
...
4. weapp/docs/rider/task-cards/card-03-rider-credit-contract-rebuild.md
5. weapp/docs/rider/task-cards/card-04-rider-exception-claim-appeal-split.md
6. weapp/docs/rider/task-cards/card-05-rider-wallet-withdraw-closure.md
7. weapp/docs/rider/task-cards/card-06-rider-orphan-pages-and-legacy-manage-cleanup.md
8. weapp/docs/rider/task-cards/card-07-rider-runtime-validation-and-weak-network-regression.md

## 8. 验证基线

本轮审计阶段已完成：

1. rider 子包 8 个注册页面静态复核。

...
- [ ] 核对支付结果与订单状态的真实 contract，确认页面应承接的状态集合。
- [ ] 把结果页拆成成功、失败、取消、未知结果等关键状态分支，而不是单一成功态。
- [ ] 为未知结果提供回查入口或自动回查策略，不再把未知结果伪装成成功或失败。
- [ ] 优化结果页的信息架构，移除与主任务无关的干扰信息，让结果说明、下一步动作和恢复入口更聚焦。
- [ ] 处理用户重新进入、回前台、重复进入结果页时的状态恢复。

## 完成定义

- [ ] 结果页不再是静态成功壳，而是按真实支付/订单状态承接。
- [ ] 未知结果有明确回查路径。
- [ ] 用户从微信回跳或弱网重入后不会被错误地落在成功页。

## 验证要求

- [ ] 人工验证成功、失败、取消、未知结果四类主要场景。
- [ ] 验证微信回跳、回前台、重新打开结果页时的恢复表现。
- [ ] review 时使用 `.github/standards/weapp/REVIEW_CHECKLIST.md`，重点检查反馈与提示、状态恢复和契约闭环。

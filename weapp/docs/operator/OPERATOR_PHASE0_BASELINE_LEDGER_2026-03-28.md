...
> 历史快照说明
>
> 本文是 2026-03-28 审计阶段的运营侧基线台账快照，主要用于保留当时的诊断上下文、页面分布和执行起点，不代表当前 operator 子包的现行页面数量、入口结构、接口承接或最终评分。
>
> 涉及当前实现时，请优先以实际代码、当前注册配置、后端真实路由，以及 weapp/docs/operator 下的最新任务卡和后续收口文档为准。

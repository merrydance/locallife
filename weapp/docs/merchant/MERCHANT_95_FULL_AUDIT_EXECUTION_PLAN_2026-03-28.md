...
> 历史快照说明
>
> 本文是 2026-03-28 审计阶段形成的商户侧执行计划快照，保留的是当时的问题判断、分阶段收口路径和风险排序，不代表当前 merchant 子包的现行页面结构、媒体合同、页面职责或最终交付状态。
>
> 涉及当前真值时，请优先以 weapp/miniprogram 的实际页面代码、当前注册配置、后端真实路由，以及 weapp/docs/merchant 下的最新任务卡和后续收口文档为准。

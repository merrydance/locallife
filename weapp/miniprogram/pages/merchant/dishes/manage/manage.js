...
Page({
    ...
    async loadDishes() {
        try {
            ...
            const result = await (0, merchant_dish_combo_management_1.getDishes)(params);
            this.setData({
                dishes: result.data
            });
        }
    ...
    async batchOperation(e) {
        ...
        try {
            switch (operation) {
                case 'enable':
                    await (0, merchant_dish_combo_management_1.batchUpdateDishStatus)({ dish_ids: selectedDishes, is_online: true });
                    wx.showToast({ title: '批量上架成功', icon: 'success' });
                    break;
                case 'disable':
                    await (0, merchant_dish_combo_management_1.batchUpdateDishStatus)({ dish_ids: selectedDishes, is_online: false });
     ...
     * 批量删除菜品
     */
    async batchDeleteDishes(dishIds) {
        return new Promise((resolve, reject) => {
            wx.showModal({
                ...
                success: async (res) => {
                    if (res.confirm) {
                        try {
                            await Promise.all(dishIds.map(id => (0, merchant_dish_combo_management_1.deleteDish)(id)));
    ...
    async toggleDishStatus(e) {
        ...
        try {
            await (0, merchant_dish_combo_management_1.updateDishStatus)(id, { is_available: !available });
            wx.showToast({
    ...
    async uploadImage(filePath) {
        try {
            ...
            const result = await (0, merchant_basic_management_1.uploadImage)(filePath, 'dish');
            this.setData({
    ...
    async saveDish() {
        ...
        try {
            ...
            if (editingDish) {
                // 更新菜品
                await (0, merchant_dish_combo_management_1.updateDish)(editingDish.id, dishData);
                wx.showToast({ title: '更新成功', icon: 'success' });
            }
            else {
                // 创建菜品
                await (0, merchant_dish_combo_management_1.createDish)(dishData);
    ...
    deleteDish(e) {
        ...
        wx.showModal({
            ...
            success: async (res) => {
                if (res.confirm) {
                    try {
                        await (0, merchant_dish_combo_management_1.deleteDish)(dishId);
    ...
    async manageInventory(e) {
        ...
        try {
            // 获取当前库存
            const inventory = await (0, merchant_dish_combo_management_1.getInventory)(dishId);
            this.setData({
    ...
    async saveInventoryAdjustment() {
        ...
        try {
            wx.showLoading({ title: '保存中...' });
            await (0, merchant_dish_combo_management_1.updateInventory)(inventoryForm.dish_id, {

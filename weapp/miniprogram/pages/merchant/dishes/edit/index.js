...
Page({
    ...
    async loadCategoriesAndDish(dishId) {
        this.setData({ loading: true });
        try {
            const categories = await dish_1.DishManagementService.getDishCategories();
            this.setData({ categories });
            if (dishId) {
                const dish = await dish_1.DishManagementService.getDishDetail(dishId);
                if (dish) {
                    this.setData({
                        form: {
                            name: dish.name,
    ...
    async onSubmit() {
        ...
        try {
            ...
            if (isEdit) {
                await dish_1.DishManagementService.updateDish(dishId, payload);
            }
            else {
                await dish_1.DishManagementService.createDish(payload);
            }

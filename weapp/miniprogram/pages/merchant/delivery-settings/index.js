...
Page({
    ...
    async loadPromotions() {
        ...
        try {
            ...
            const result = await delivery_fee_1.deliveryFeeService.getMerchantPromotions(merchantId);
            // 格式化显示数据
    ...
    async createPromotion() {
        ...
        try {
            const merchantId = Number(app.globalData.merchantId);
            await delivery_fee_1.deliveryFeeService.createMerchantPromotion(merchantId, {
        ...
        wx.showModal({
            ...
            success: async (res) => {
                if (res.confirm) {
                    try {
                        const merchantId = Number(app.globalData.merchantId);
                        await delivery_fee_1.deliveryFeeService.deleteMerchantPromotion(merchantId, promoId);

...
import {
  ...
  formatTime,
  getMerchantClaimProgressCurrent,
  isClaimRecoveryNotFoundError,
  toUserRiskView,
  type BehaviorSummaryCardView,
  type MerchantClaimDetailView,
  type MerchantUserRiskView
...
Page({
  data: {
    ...
    riskError: false,
    riskErrorMessage: '',
    userRisk: null as MerchantUserRiskView | null,
  ...
  async loadDetail(silent = false, preserveActionNotice = false) {
    ...
    try {
      ...
      const [decisionResult, recoveryResult, appealResult, behaviorResult, riskResult] = await settleAll([
        claimManagementService.getMerchantClaimDecision(this.data.claimId),
        recoveryId ? claimManagementService.getMerchantClaimRecovery(recoveryId) : Promise.resolve(null as ClaimRecoveryResponse | null),
        appealId ? appealManagementService.getMerchantAppealDetail(appealId) : Promise.resolve(null as AppealResponse | null),
        claimManagementService.getMerchantClaimBehaviorSummary(claim.order_id),
        claimManagementService.getMerchantUserRisk(claim.user_id)
      ] as const)
      ...
      const userRisk = isSettledFulfilled(riskResult)
        ? toUserRiskView(riskResult.value)
        : null
      ...
      const detail: MerchantClaimDetailView = {
        ...
        canSubmitAppeal: canSubmitMerchantClaimAppeal(claim.status, appealId),
        canPayRecovery: Boolean(recoveryId && recovery && canPayClaimRecovery(recovery.status)),
        progressCurrent: getMerchantClaimProgressCurrent(claim.status, recovery?.status, Boolean(appealId)),
        progressClaimText: formatTime(claim.created_at),
        progressRecoveryText: recovery?.due_at ? formatTime(recovery.due_at) : '等待平台生成或无需追偿',
        progressAppealText: appeal?.reviewed_at ? formatTime(appeal.reviewed_at) : (appeal?.created_at ? formatTime(appeal.created_at) : '未提交异议')
      }

      const nextBehaviorCards = behaviorSummary ? buildBehaviorCards(behaviorSummary) : []
      ...
      this.setData({
        ...
        behaviorCards: nextBehaviorCards,
        hasRiderBehavior: Boolean(behaviorSummary?.rider),
        riskLoading: false,
        riskError: isSettledRejected(riskResult),
        riskErrorMessage: isSettledRejected(riskResult) ? '顾客风险提示加载失败，可单独重试' : '',
        userRisk
      })
    } catch (error) {
      logger.error('Load merchant claim detail failed', error)
  ...
  applyAppealSubmitted(appeal: AppealResponse, reason: string) {
    ...
    this.setData({
      ...
      actionNoticeMessage: '异议已提交，平台复核状态稍后同步',
      actionNoticeRefreshable: false,
      refreshErrorMessage: ''
    })
  },
  ...
  async onRetryDecision() {
    ...
    try {
      const result = await claimManagementService.getMerchantClaimDecision(this.data.claimId)
      ...
      logger.error('Reload merchant claim decision failed', error)
      this.setData({
        decisionLoading: false,
        decisionError: true,
  ...
  async onRetryRecovery() {
    ...
    try {
      const recovery = await claimManagementService.getMerchantClaimRecovery(this.data.recoveryId)
      this.setData({
        recoveryLoading: false,
        recoveryError: false,
        recoveryErrorMessage: '',
      ...
      logger.error('Reload merchant claim recovery failed', error)
  ...
  async onRetryBehavior() {
    ...
    try {
      const summary = await claimManagementService.getMerchantClaimBehaviorSummary(orderId)
      this.setData({
        behaviorLoading: false,
        behaviorError: false,
        behaviorErrorMessage: '',
      ...
      logger.error('Load merchant claim behavior summary failed', error)
      this.setData({
        behaviorLoading: false,
        behaviorError: true,
  ...
  async onRetryRisk() {
    ...
    if (!userId || this.data.riskLoading) return
    ...
    try {
      const risk = await claimManagementService.getMerchantUserRisk(userId)
      this.setData({
        riskLoading: false,
        riskError: false,
        riskErrorMessage: '',
        userRisk: toUserRiskView(risk)
      })
    } catch (error) {
      logger.error('Load merchant user risk failed', error)
      this.setData({
        riskLoading: false,
        riskError: true,
        riskErrorMessage: getErrorMessage(error, '顾客风险提示加载失败，可稍后重试')
      })
    }
  },
  ...
  async onSubmitAppeal() {
    ...
    try {
      ...
      const appeal = await appealManagementService.createAppeal({
  ...
  async onPayRecovery() {
    ...
    try {
      ...
      const paymentResult = await claimManagementService.payMerchantClaimRecovery(this.data.recoveryId)
  ...
  onViewAppealDetail() {
    const appealId = this.data.detail?.appealId
    if (!appealId) return

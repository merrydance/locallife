...
interface MerchantUserRiskView {
  hasBlock: boolean
  reasonCode?: string
...
Page({
  ...
  async loadDetail(silent = false) {
    ...
    try {
      ...
      const userRisk = riskResult.status === 'fulfilled'
        ? this.toUserRiskView(riskResult.value)
        : null
      ...
      const detail: MerchantClaimDetailView = {
        ...
        canSubmitAppeal: claim.status === 'approved' && !claim.appeal_id,
      ...
      this.setData({
        ...
        riskLoading: false,
        riskError: riskResult.status === 'rejected',
        riskErrorMessage: riskResult.status === 'rejected' ? '顾客风险提示加载失败，可单独重试' : '',
        userRisk
      })
    } catch (error) {
  ...
  toUserRiskView(risk: MerchantUserRiskResponse): MerchantUserRiskView {
    return {
  ...
  async onRetryRisk() {
    ...
    try {
      ...
      this.setData({
        ...
        riskError: false,
        riskErrorMessage: '',
        userRisk: this.toUserRiskView(risk)
      })
    } catch (error) {

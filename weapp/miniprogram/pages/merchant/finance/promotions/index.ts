import {
  ...
  type MerchantPromotionExpenseItem,
...
const EMPTY_SUMMARY: MerchantPromotionExpensesResponse = {
  orders: [],
  total: 0,
  page: 1,
  limit: 20,
...
Page({
  data: {
    ...
    dateRangeLabel: '--',
    updatedAtLabel: '--',
    summary: EMPTY_SUMMARY as MerchantPromotionExpensesResponse,
    orders: [] as MerchantPromotionExpenseItem[]
  },
  ...
  async bootstrapPage() {
    ...
    if (access.status !== 'granted') {
      this.setData({
        accessReady: true,
        accessDenied: true,
        accessErrorMessage: access.message || '当前账号暂时不能查看营销支出',
  ...
  async loadData(options?: { force?: boolean }) {
    ...
    try {
      const response = await getMerchantPromotionExpenses({ ...params, page: 1, limit: 20 })
      this.setData({
        summary: response,
      ...
      logger.error('Load merchant finance promotions failed', error, 'merchant-finance-promotions')
      const message = getErrorUserMessage(error, '营销支出加载失败，请稍后重试')
      this.setData({
        loading: false,
        initialLoading: false,

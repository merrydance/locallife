...
function buildWithdrawalIdempotencyKey() {
  return `merchant-withdrawal:${Date.now()}:${Math.random().toString(36).slice(2, 10)}`
}
...
Page({
  ...
  async loadBalance() {
    ...
    try {
      const balance = await getBaofuWithdrawalBalance('merchant')
      this.setData({
      ...
      logger.warn('Merchant baofu withdrawal balance load failed', error)
      this.setData({
        loadingBalance: false,
        initialLoading: false,
        initialError: true,
  ...
  async onSubmit() {
    ...
    try {
      const result = await createBaofuWithdrawal('merchant', { amount: check.amount }, { idempotencyKey })
      ...
      logger.warn('Submit merchant baofu withdrawal failed', error)
      wx.showToast({

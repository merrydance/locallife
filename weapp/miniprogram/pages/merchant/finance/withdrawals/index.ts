...
Page({
  ...
  async loadWithdrawals(options: { silent?: boolean, append?: boolean, page?: number } = {}) {
    ...
    try {
      ...
      logger.warn('Merchant baofu withdrawals load failed', error)
  ...
  async fetchWithdrawals(page: number): Promise<MerchantWithdrawalFetchResult> {
    const [balanceResult, recordsResult] = await Promise.all([
      settleBaofuWithdrawalRequest(getBaofuWithdrawalBalance('merchant')),
      settleBaofuWithdrawalRequest(listBaofuWithdrawals('merchant', { page, limit: PAGE_SIZE }))
    ])

    if (isBaofuWithdrawalRequestRejected(balanceResult)) {
      logger.warn('Merchant baofu withdrawal balance load failed', balanceResult.reason)
    }
    if (isBaofuWithdrawalRequestRejected(recordsResult)) {
      logger.warn('Merchant baofu withdrawals records load failed', recordsResult.reason)
    }

...
Page({
    ...
    loadKitchenOrders() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                // 1. 新订单：按创建时间正序 (FIFO)
                const newOrders = (result.new_orders || []).sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
                // 2. 制作中：按剩余时间正序 (紧急的在前)
                const preparingOrders = (result.preparing_orders || []).sort((a, b) => {
                    const remainingA = order_management_1.OrderManagementAdapter.getRemainingTime(a);
                    const remainingB = order_management_1.OrderManagementAdapter.getRemainingTime(b);
                    return remainingA - remainingB;
                });
                // 3. 待取餐：按完成时间倒序 (最新完成的在前)
                const readyOrders = (result.ready_orders || []).sort((a, b) => new Date(b.ready_at).getTime() - new Date(a.ready_at).getTime());
                this.setData({
                    newOrders,
                    preparingOrders,
                    readyOrders,
    ...
    viewOrderDetail(e) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const order = yield order_management_1.KitchenDisplayService.getKitchenOrderDetail(orderId);
                this.setData({
     ...
     * 检查超时订单
     */
    checkOverdueOrders(preparingOrders) {
        const overdueOrders = preparingOrders.filter(order => order_management_1.OrderManagementAdapter.isOrderOverdue(order));
    ...
    getRemainingTime(order) {
        const remaining = order_management_1.OrderManagementAdapter.getRemainingTime(order);
        if (remaining <= 0) {
            return '已超时';
        }
    ...
    isOrderOverdue(order) {
        return order_management_1.OrderManagementAdapter.isOrderOverdue(order);
    },
    /**
     * 格式化时间
     */

...
Page({
    ...
    async loadStats() {
        try {
            ...
            const stats = await reservation_1.ReservationService.getReservationStats();
            this.setData({ stats });
        }
        catch (error) {
    ...
    async loadReservations() {
        ...
        try {
            ...
            const result = await reservation_1.ReservationService.getMerchantReservations(params);
    ...
    async onSubmitCreate() {
        ...
        try {
            await reservation_1.ReservationService.merchantCreateReservation(createForm);
            wx.hideLoading();
    ...
    async onCheckIn(e) {
        const id = e.currentTarget.dataset.id;
        try {
            await reservation_1.ReservationService.checkIn(id);
        ...
        wx.showModal({
            ...
            success: async (res) => {
                if (res.confirm) {
                    ...
                    try {
                        await reservation_1.ReservationService.cancelReservation(id, '商户主动取消');
                        wx.hideLoading();

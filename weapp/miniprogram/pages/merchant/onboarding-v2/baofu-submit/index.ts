...
import type { ApplymentBankFormDraftPayload, ApplymentBankFormPayload } from '../../finance/settlement-account/submit/_components/applyment-bank-form/index'
...
const V2_PROGRESS_PATH = '/pages/merchant/onboarding-v2/index'
...
async function merchantAccessGuard(): Promise<AccessCheckResult> {
  ...
  if (accessResult.status === 'error') {
    logger.error('Merchant onboarding v2 baofu submit access check failed action=check_access role=merchant', accessResult.message, 'merchant-onboarding-v2-baofu-submit')
  }
...
Page({
  behaviors: [
    baofuSettlementSubmitBehavior({
      role: 'merchant',
      statusPagePath: '/pages/merchant/onboarding-v2/index?from=baofu_submit',
      getAccount: getMerchantBaofuSettlementAccount,
      accessGuard: merchantAccessGuard,
      logTag: 'merchant-onboarding-v2-baofu-submit',
  ...
  data: {
    ...
    accountOpeningModeOptions: [
      { value: 'business', label: '营业执照开户' },
      { value: 'personal', label: '个人开户' }
    ],
  ...
  onOpeningModeChange(e: WechatMiniprogram.TouchEvent) {
    ...
    this.setData({ accountOpeningMode: value })
  },
  ...
  onBackToProgress() {
    wx.redirectTo({ url: V2_PROGRESS_PATH })
  },
  ...
  async submitCurrentProfile(bankPayload: ApplymentBankFormPayload) {
    ...
    try {
      ...
      logger.error('Submit merchant onboarding v2 baofu profile failed action=submit_profile role=merchant', error, 'merchant-onboarding-v2-baofu-submit')

...
Page({
    ...
    async loadMerchantInfo() {
        try {
            const info = await merchant_1.MerchantManagementService.getMerchantInfo();
            if (info) {
                this.setData({
    ...
    async loadStats() {
        try {
            ...
            const stats = await merchant_analytics_1.MerchantStatsService.getStatsOverview({
                start_date: today,
                end_date: today
            });
            if (stats) {
    ...
    async loadOrders() {
        try {
            const orders = await order_management_1.MerchantOrderManagementService.getOrderList({
                page_id: 1,
                page_size: 50
            });
            // 订单类型和状态映射
    ...
    async loadTables() {
        try {
            const response = await (0, merchant_table_device_management_1.getTables)({});
    ...
    async connectWebSocket() {
        ...
        try {
            await websocket_realtime_1.RealtimeUtils.initializeForMerchant(Number(userId || 0), Number(merchantId), {
                onOpen: () => {
    ...
    async onToggleStatus() {
        ...
        try {
            await merchant_1.MerchantManagementService.updateMerchantStatus({ is_open: newStatus });
        ...
        try {
            ...
            if (occupiedTables.length > 0) {
                ...
                for (const table of occupiedTables) {
                    await (0, merchant_table_device_management_1.updateTableStatus)(table.id, 'available');
                }
    ...
    async setTableStatus(e) {
        const newStatus = e.currentTarget.dataset.status;
        const { activeTable } = this.data;
        if (!(activeTable === null || activeTable === void 0 ? void 0 : activeTable.id))
            return;
        try {
            await (0, merchant_table_device_management_1.updateTableStatus)(activeTable.id, newStatus);

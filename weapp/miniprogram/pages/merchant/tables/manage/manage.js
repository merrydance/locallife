...
Page({
    ...
    async loadTables() {
        ...
        try {
            const result = await (0, merchant_table_device_management_1.getTables)({ page: 1, page_size: 100 });
            const processedTables = result.tables.map((t) => ({
                ...t,
    ...
    async onSaveTable() {
        ...
        try {
            if (isAdding) {
                await (0, merchant_table_device_management_1.createTable)(selectedTable);
                wx.showToast({ title: '新增成功' });
            }
            else {
                await (0, merchant_table_device_management_1.updateTable)(selectedTable.id, selectedTable);
    ...
    async onRefreshQRCode() {
        ...
        try {
            const res = await (0, merchant_table_device_management_1.generateTableQRCode)(this.data.selectedTable.id);
    ...
    async onDeleteTable() {
        ...
        if (res.confirm) {
            ...
            try {
                await (0, merchant_table_device_management_1.deleteTable)(selectedTable.id);

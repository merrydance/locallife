...
Page({
    ...
    async loadCombos() {
        ...
        try {
            ...
            const response = await dish_1.ComboManagementService.listCombos({
                page_id: 1,
                page_size: 50
            });
    ...
    async loadComboDetail(id) {
        var _a, _b;
        const requestedId = id; // 记录本次请求的套餐ID
        try {
            const detail = await dish_1.ComboManagementService.getComboDetail(id);
            ...
            // 更新完整数据
            this.setData({
                selectedCombo: { ...detail, dish_count: ((_b = detail.dishes) === null || _b === void 0 ? void 0 : _b.length) || 0 },
    ...
    async onSaveCombo() {
        ...
        try {
            if (isAdding) {
                ...
                await dish_1.ComboManagementService.createCombo({
                ...
                await dish_1.ComboManagementService.updateCombo(selectedCombo.id, {
                    name: selectedCombo.name.trim(),
                    description: selectedCombo.description || '',
                    combo_price: selectedCombo.combo_price,
    ...
    async onDeleteCombo() {
        ...
        if (res.confirm) {
            ...
            try {
                await dish_1.ComboManagementService.deleteCombo(selectedCombo.id);
                wx.hideLoading();

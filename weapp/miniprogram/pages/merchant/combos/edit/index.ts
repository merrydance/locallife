...
import {
  ...
  buildDishOptions,
  buildSelectedComboDishes,
  buildSelectedDishMap,
  buildSelectedSpecState,
  buildSelectedTagIds,
...
Page({
  ...
  async loadData() {
    ...
    try {
      ...
      if (!isSettledFulfilled(allDishesResult)) {
        throw allDishesResult.reason
      }
      if (this.data.isEdit && !isSettledFulfilled(comboResult)) {
        throw comboResult.reason
      }

      const allDishesResponse = allDishesResult.value
      const comboRes = isSettledFulfilled(comboResult) ? comboResult.value : null
      const availableTags = isSettledFulfilled(tagsResult) ? tagsResult.value : []
      const loadWarningMessage = !isSettledFulfilled(tagsResult)
        ? '套餐标签未同步，仍可继续编辑套餐内容'
        : ''

      const dishes = buildDishOptions(allDishesResponse)

      const selectedDishMap = buildSelectedDishMap(comboRes)
      ...
      this.setData({
        ...
        availableTags,
        selectedTagIds: buildSelectedTagIds(comboRes?.tags),
        selectedTagState: buildSelectedTagState(buildSelectedTagIds(comboRes?.tags)),
  ...
  async onConfirmCreateTagDialog() {
    ...
    const exists = this.data.availableTags.some((tag) => tag.name.trim() === name)
    if (exists) {
    ...
    try {
      ...
      const nextTagIds = this.data.selectedTagIds.includes(created.id)
        ? this.data.selectedTagIds
      ...
      wx.showToast({ title: getErrorMessage(err, '新增套餐标签失败，请重试'), icon: 'none' })
    } finally {

...
Page({
    ...
    async loadSettings() {
        try {
            const settings = await marketing_membership_1.membershipSettingsService.getMembershipSettings();
    ...
    async loadRechargeRules() {
        ...
        if (!merchantId)
            return;
        try {
            const rules = await marketing_membership_1.rechargeRuleManagementService.listRechargeRules(merchantId);
            // 预处理价格
            const processedRules = rules.map(rule => ({
                ...rule,
                recharge_amount_display: (0, util_1.formatPriceNoSymbol)(rule.recharge_amount || 0),
                bonus_amount_display: (0, util_1.formatPriceNoSymbol)(rule.bonus_amount || 0),
    ...
    async onSaveSettings() {
        ...
        try {
            ...
            await marketing_membership_1.membershipSettingsService.updateMembershipSettings(request);
    ...
    async onSaveRule() {
        ...
        try {
            if (editingRule) {
                ...
                await marketing_membership_1.rechargeRuleManagementService.updateRechargeRule(merchantId, editingRule.id, request);
            }
            else {
                // 创建
                const request = {
                ...
                await marketing_membership_1.rechargeRuleManagementService.createRechargeRule(merchantId, request);
            }
            wx.hideLoading();
            wx.showToast({ title: '保存成功', icon: 'success' });
        ...
        wx.showModal({
            ...
            success: async (res) => {
                if (res.confirm) {
                    ...
                    try {
                        await marketing_membership_1.rechargeRuleManagementService.deleteRechargeRule(this.data.merchantId, rule.id);
                        wx.hideLoading();
    ...
    async onToggleRuleStatus(e) {
        ...
        try {
            await marketing_membership_1.rechargeRuleManagementService.updateRechargeRule(this.data.merchantId, rule.id, { is_active: newStatus });

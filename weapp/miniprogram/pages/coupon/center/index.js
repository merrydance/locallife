...
Page({
    ...
    async loadCoupons(reset) {
        ...
        try {
            ...
            const res = await coupon_1.CouponService.getAvailableCoupons({
                page_id: page,
                page_size: this.data.pageSize
            });
            const newCoupons = res.coupons.map(c => ({
                ...c,
                valueDisplay: c.type === 'discount' ? String(c.value / 10) : (0, util_1.formatPriceNoSymbol)(c.value || 0),
                _formatValue: c.type === 'discount' ? `${c.value / 10}折` : `¥${(0, util_1.formatPriceNoSymbol)(c.value || 0)}`,
                _formatMinSpend: c.min_spend > 0 ? `满${(0, util_1.formatPriceNoSymbol)(c.min_spend)}可用` : '无门槛',
    ...
    async onClaim(e) {
        ...
        try {
            ...
            await coupon_1.CouponService.claimCoupon(id);
            // Update local state

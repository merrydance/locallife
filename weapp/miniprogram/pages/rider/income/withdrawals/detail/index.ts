...
const WITHDRAWAL_LIST_PAGE_PATH = '/pages/rider/income/withdrawals/index'
...
Page({
  ...
  async loadDetail(options: { silent?: boolean } = {}) {
    ...
    try {
      const response = await getBaofuWithdrawal('rider', this.data.id)
      this.setData({
      ...
      logger.warn('Rider income baofu withdrawal detail load failed', error)
      const message = getErrorUserMessage(error, '收入提现详情加载失败，请稍后重试')
      this.setData({
        initialLoading: false,

...
function buildWithdrawalIdempotencyKey() {
  return `rider-income-withdrawal:${Date.now()}:${Math.random().toString(36).slice(2, 10)}`
}
...
Page({
  ...
  async loadBalance() {
    ...
    try {
      const balance = await getBaofuWithdrawalBalance('rider')
      this.setData({
      ...
      logger.warn('Rider income baofu withdrawal balance load failed', error)
      this.setData({
        loadingBalance: false,
        initialLoading: false,
        initialError: true,
        initialErrorMessage: getErrorUserMessage(error, '可提现收入加载失败，请稍后重试')
      })
    }
  },
  ...
  async onSubmit() {
    ...
    try {
      const result = await createBaofuWithdrawal('rider', { amount: check.amount }, { idempotencyKey })
      ...
      logger.warn('Submit rider income baofu withdrawal failed', error)
      wx.showToast({
        title: getErrorUserMessage(error, '收入提现申请失败，请稍后重试'),
        icon: 'none'
      })

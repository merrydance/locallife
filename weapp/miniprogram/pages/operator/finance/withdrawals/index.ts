...
Page({
  ...
  async fetchWithdrawals(page: number): Promise<OperatorWithdrawalFetchResult> {
    const [balanceResult, recordsResult] = await Promise.all([
      settleBaofuWithdrawalRequest(getBaofuWithdrawalBalance('operator')),
      settleBaofuWithdrawalRequest(listBaofuWithdrawals('operator', { page, limit: PAGE_SIZE }))
    ])

    if (isBaofuWithdrawalRequestRejected(balanceResult)) {
      logger.warn('Operator baofu withdrawal balance load failed', balanceResult.reason)
    }
    if (isBaofuWithdrawalRequestRejected(recordsResult)) {
      logger.warn('Operator baofu withdrawals records load failed', recordsResult.reason)
    }

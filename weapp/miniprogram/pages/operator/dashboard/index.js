...
Page({
    ...
    async loadDashboardData() {
        ...
        try {
            // 获取实时大盘数据
            const realtimeData = await platform_dashboard_1.platformDashboardService.getRealtimeDashboard();
            this.setData({
                stats: {
    ...
    async loadPendingApprovals() {
        try {
            // 获取待审批商户列表
            const merchantList = await operator_merchant_management_1.operatorMerchantManagementService.getMerchantList({
                page_id: 1,
                page_size: 10
            });
            // 筛选待审批的商户
    ...
    async onApprove(e) {
        ...
        try {
            await operator_merchant_management_1.operatorMerchantManagementService.resumeMerchant(id, { reason: '审批通过' });
    ...
    async onReject(e) {
        const { id } = e.currentTarget.dataset;
        try {
            await operator_merchant_management_1.operatorMerchantManagementService.suspendMerchant(id, { reason: '审批拒绝' });

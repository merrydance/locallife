...
Page({
    ...
    async loadMerchants(reset = true) {
        ...
        try {
            const result = await operator_merchant_management_1.operatorMerchantManagementService.getMerchantList({
        ...
        wx.showModal({
            ...
            success: async (res) => {
                if (res.confirm) {
                    try {
                        if (isActive) {
                            await operator_merchant_management_1.operatorMerchantManagementService.suspendMerchant(id, { reason: '运营封禁' });
                        }
                        else {
                            await operator_merchant_management_1.operatorMerchantManagementService.resumeMerchant(id, { reason: '运营解封' });
                        }

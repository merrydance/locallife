...
Page({
    ...
    async loadDishDetail() {
        ...
        try {
            ...
            // 获取菜品详情
            const dishData = await dish_1.DishManagementService.getDishDetail(dishId);
            if (!dishData) {
            ...
            if (dishData.merchant_id) {
                try {
                    const reviewsResult = await (0, personal_1.getMerchantReviews)(dishData.merchant_id, {
                        page_id: 1,
                        page_size: 5
                    });
    ...
    async onAddToCart() {
        ...
        tracker_1.tracker.log(tracker_1.EventType.ADD_CART, String(dish.id), {
            shop_id: dish.shop_id,
            quantity,
            price: totalPrice,
            tags: dish.tags

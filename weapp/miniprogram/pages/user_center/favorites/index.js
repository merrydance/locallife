...
Page({
    ...
    async loadFavoriteDishes() {
        const result = await (0, personal_1.getFavoriteDishes)({ page: 1, page_size: 50 });
    ...
    async loadFavoriteMerchants() {
        const result = await (0, personal_1.getFavoriteMerchants)({ page: 1, page_size: 50 });
    ...
    async doRemoveFavorite(id, type) {
        wx.showLoading({ title: '处理中...' });
        try {
            if (type === 'DISH') {
                await (0, personal_1.removeDishFromFavorites)(id);
            }
            else {
                await (0, personal_1.removeMerchantFromFavorites)(id);
            }
            wx.hideLoading();

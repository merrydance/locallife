...
Page({
    ...
    async initPage() {
        ...
        try {
            ...
            // 获取商户信息
            const merchantInfo = await (0, merchant_1.getPublicMerchantDetail)(merchantId);
            // 获取桌台/预约信息
            let tableNo = '';
            let reservationInfo = null;
            if (this.data.orderType === 'dine_in' && this.data.tableId) {
                const tableResult = await (0, table_1.getTableDetail)(this.data.tableId);
                tableNo = tableResult.table_no;
            }
            else if (this.data.orderType === 'reservation' && this.data.reservationId) {
                reservationInfo = await (0, reservation_1.getReservationDetail)(this.data.reservationId);
                tableNo = reservationInfo.table_no || '';
            }
            // 预处理购物车数据
            const processedItems = (cart.items || []).map(item => ({
                ...item,
                dish_image: (0, image_1.getPublicImageUrl)(item.image_url),
                priceDisplay: (0, util_1.formatPriceNoSymbol)(item.unit_price || 0),
                subtotalDisplay: (0, util_1.formatPriceNoSymbol)(item.subtotal || 0)
            }));
            const processedCalculation = {
                ...calculation,
                subtotalDisplay: (0, util_1.formatPriceNoSymbol)(calculation.subtotal || 0),
                discountDisplay: (0, util_1.formatPriceNoSymbol)(calculation.discount_amount || 0),
                totalDisplay: (0, util_1.formatPriceNoSymbol)(calculation.total_amount || 0)
            };
            // 预处理商户信息
            const processedMerchant = {
                ...merchantInfo,
                logo_url: (0, image_1.getPublicImageUrl)(merchantInfo.logo_url)
            };
            // 获取用户在该商户的储值余额
            let memberBalance = 0;
            let memberBalanceDisplay = '0.00';
            try {
                const membershipsResult = await (0, personal_1.getMyMemberships)();
    ...
    async onShowVouchers() {
        ...
        try {
            const result = await coupon_1.CouponService.getMyCoupons({
                status: 'available',
                page_id: 1,
                page_size: 50
            });
    ...
    async onClearVoucher() {
        ...
        try {
            ...
            const calculation = await (0, cart_1.calculateCart)(params);
            this.setData({
                calculation: {
                    ...calculation,
                    subtotalDisplay: (0, util_1.formatPriceNoSymbol)(calculation.subtotal || 0),
    ...
    async onSelectVoucher(e) {
        const { voucher } = e.currentTarget.dataset;
        if (!voucher)
            return;
        this.setData({
    ...
    async onSubmitOrder() {
        ...
        try {
            ...
            const order = await (0, order_1.createOrder)(orderData);
            // 创建支付
     ...
     * 创建支付
     */
    async doCreatePayment(orderId, _amount, paymentMethod) {
        try {
            if (paymentMethod === 'wechat_pay') {
                // 创建支付订单
                const paymentResult = await (0, payment_1.createPayment)({
                    order_id: orderId,
     ...
     * 处理支付宝支付
     */
    async handleAlipay(_paymentResult) {
        // 支付宝支付逻辑
        // 这里需要根据实际的支付宝SDK实现
        wx.showToast({
            title: '支付宝支付暂未开放',

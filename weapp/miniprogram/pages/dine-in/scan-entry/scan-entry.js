...
Page({
    ...
    async loadTableInfo(tableId) {
        try {
            ...
            const detail = await (0, table_1.getTableDetail)(tableId);
            const scanResult = await (0, table_1.scanTable)(detail.merchant_id, detail.table_no);
            this.setData({
                loading: false,
                tableInfo: {
                    id: detail.id,
    ...
    async loadTableInfoByNo(merchantId, tableNo) {
        try {
            this.setData({ loading: true, error: null });
    ...
    async confirmTransfer() {
        ...
        try {
            await (0, dining_session_1.transferDiningSessionTable)(activeSession.id, {
     ...
     * 记录扫码行为
     */
    async trackScanBehavior(tableId, merchantId) {
        try {
            // 记录用户行为用于数据分析

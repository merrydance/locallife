 ...
 * @param paymentId 支付订单ID
 */
async function closePayment(paymentId) {
    return (0, request_1.request)({
 ...
 */
async function createRefund(paymentId, refundData) {
    return (0, request_1.request)({
        url: `/v1/payments/${paymentId}/refunds`,
        method: 'POST',
        data: refundData
 ...
 */
async function createReservationPayment(reservationId) {
    return createPayment({
 ...
 */
async function processPayment(orderId, businessType = 'order') {
    try {
        // 1. 创建支付订单
 ...
 */
async function pollPaymentStatus(paymentId, maxAttempts = 30, interval = 2000) {

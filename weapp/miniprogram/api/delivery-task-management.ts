 ...
 */
export class DeliveryTaskManagementAdapter {
    /**
     * 适配推荐订单响应数据
     */
    static adaptRecommendedOrderResponse(data: RecommendedOrderResponse): {
        orderId: number
        merchantId: number
        deliveryFee: number
     ...
     * 适配配送响应数据
     */
    static adaptDeliveryResponse(data: DeliveryResponse): {
        id: number
        orderId: number
        riderId: number
        status: DeliveryStatus
 ...
 */
export function getSmartOrderRecommendations(
    orders: RecommendedOrderResponse[],
    preferences: {
        maxDistance?: number
    ...
    highPriority: RecommendedOrderResponse[]
    recommended: RecommendedOrderResponse[]
    others: RecommendedOrderResponse[]
} {
    const highPriority: RecommendedOrderResponse[] = []
 ...
 */
export function calculateDeliveryEfficiency(deliveries: DeliveryResponse[]): {
    avgDeliveryTime: number
    onTimeRate: number
    totalDistance: number
    totalEarnings: number
...
export class DeliveryReminderManager {
     ...
     * @param callback 提醒回调
     */
    setDeliveryReminder(delivery: DeliveryResponse, callback: (delivery: DeliveryResponse, type: string) => void): void {
        // 清除已有提醒
     ...
     * @param deliveryId 配送任务ID
     */
    clearReminder(deliveryId: number): void {
 ...
 */
export function calculateEstimatedArrival(startTime: string, estimatedMinutes: number): string {

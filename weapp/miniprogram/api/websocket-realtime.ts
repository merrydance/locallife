...
export interface WebSocketEventHandlers {
  ...
  onOrderUpdate?: (orderData: unknown) => void
  onDeliveryUpdate?: (deliveryData: unknown) => void
...
export class WebSocketManager {
  ...
  private buildWebSocketUrl(): string {
    ...
    if (this.lastSequence > 0) {
      queryParts.push(`last_sequence=${this.lastSequence}`)
    }
  ...
  private handleMessage(message: WebSocketMessage): void {
    ...
    switch (message.type) {
      ...
      case 'location_update':
        this.eventHandlers.onLocationUpdate?.(
          message.data as RiderLocationUpdate
        )
        break
      case 'notification':
...
export class RealtimeUtils {
  ...
  static async initializeForMerchant(
    ...
    return WebSocketUtils.initializeConnection(
      'merchant',
      userId,
      {

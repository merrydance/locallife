...
exports.themeManager = exports.ThemeMode = void 0;
const logger_1 = require("./logger");
var ThemeMode;
(function (ThemeMode) {
    ThemeMode["LIGHT"] = "light";
    ThemeMode["DARK"] = "dark";
    ThemeMode["AUTO"] = "auto"; // 跟随系统
...
class ThemeManager {
    ...
    init() {
        ...
        try {
            ...
            logger_1.logger.warn('无法获取系统主题', e, 'ThemeManager');
        }
        // 读取用户偏好
        try {
            const savedMode = wx.getStorageSync(this.STORAGE_KEY);
            ...
            logger_1.logger.warn('无法读取主题偏好', e, 'ThemeManager');
        }
        // 监听系统主题变化
    ...
    setMode(mode) {
        ...
        try {
            ...
            logger_1.logger.error('保存主题偏好失败', e, 'ThemeManager');
        }
        this.applyTheme();
        logger_1.logger.info(`主题切换为: ${mode}`, undefined, 'ThemeManager');
    }
    /**
     * 获取当前模式
     */
    ...
    applyTheme() {
        ...
        try {
            ...
            logger_1.logger.warn('应用主题失败', e, 'ThemeManager');
        }
        // 在 app.wxss 中动态注入
        this.injectGlobalStyles(styleContent);
        logger_1.logger.debug('主题已应用', { isDark, colors }, 'ThemeManager');
    }
    /**
     * 生成 CSS 变量
     */
     ...
     * 注入全局样式（仅在支持的环境）
     */
    injectGlobalStyles(_styleContent) {
        // 微信小程序不支持动态注入全局样式

...
exports.imageLazyLoader = exports.ImageLoadState = exports.PLACEHOLDER_IMAGE = void 0;
const logger_1 = require("./logger");
// 占位图 (Data URL - 1x1 透明 GIF)
exports.PLACEHOLDER_IMAGE = 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7';
// 图片加载状态
var ImageLoadState;
(function (ImageLoadState) {
    ImageLoadState["IDLE"] = "idle";
...
class ImageLazyLoader {
    ...
    loadImage(url) {
        return new Promise((resolve, reject) => {
            ...
            // 如果已在加载中或已加载，跳过
            if ((cacheItem === null || cacheItem === void 0 ? void 0 : cacheItem.state) === ImageLoadState.LOADING || (cacheItem === null || cacheItem === void 0 ? void 0 : cacheItem.state) === ImageLoadState.LOADED) {
                resolve();
                return;
            }
            // 超过重试次数，跳过
            ...
            this.cache.set(url, {
                ...
                timestamp: Date.now(),
                retryCount: ((cacheItem === null || cacheItem === void 0 ? void 0 : cacheItem.retryCount) || 0)
            });
            ...
            wx.downloadFile({
                ...
                success: (res) => {
                    if (res.statusCode === 200) {
                        ...
                        logger_1.logger.debug(`图片预加载成功: ${url}`, undefined, 'ImageLazyLoader');
                    }
                    else {
                        this.handleLoadError(url);
                        logger_1.logger.warn(`图片预加载失败: ${url} HTTP ${res.statusCode}`, undefined, 'ImageLazyLoader');
                    }
                ...
                fail: (err) => {
                    this.handleLoadError(url);
                    logger_1.logger.warn(`图片预加载失败: ${url}`, err, 'ImageLazyLoader');
    ...
    getState(url) {
        var _a;
        return ((_a = this.cache.get(url)) === null || _a === void 0 ? void 0 : _a.state) || ImageLoadState.IDLE;
    }
    /**
     * 清理过期缓存
     */
    cleanup() {
        ...
        if (cleanedCount > 0) {
            logger_1.logger.debug(`清理了 ${cleanedCount} 个过期图片缓存`, undefined, 'ImageLazyLoader');
        }
    }
    /**
     * 获取缓存统计
    ...
    clear() {
        this.cache.clear();
        logger_1.logger.debug('图片缓存已清空', undefined, 'ImageLazyLoader');
    }
}
// 导出单例
exports.imageLazyLoader = new ImageLazyLoader();
// 定期清理（每10分钟）
setInterval(() => {
    exports.imageLazyLoader.cleanup();
}, 10 * 60 * 1000);

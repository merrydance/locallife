...
exports.ReservationAdapter = void 0;
class ReservationAdapter {
    ...
    static validateReservation(data) {
        if (!data.reservation_time)
            return { valid: false, message: '请选择预订时间' };
        if (!data.party_size || data.party_size <= 0)
            return { valid: false, message: '请输入正确的用餐人数' };
        if (!data.contact_name)
            return { valid: false, message: '请输入联系人姓名' };
        if (!data.contact_phone || !/^1\d{10}$/.test(data.contact_phone))
            return { valid: false, message: '请输入正确的手机号码' };
        const time = new Date(data.reservation_time).getTime();
        if (time < Date.now())
            return { valid: false, message: '预订时间不能早于当前时间' };
        return { valid: true };
    }
}
exports.ReservationAdapter = ReservationAdapter;
exports.default = ReservationAdapter;

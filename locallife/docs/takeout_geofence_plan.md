...
- 更新 swagger。
  - 已新增 TestUpdateRiderLocationGeofenceEvents 覆盖位置上报到事件触发路径。
  - Swagger 已更新。
...
## TODO（实施时勾选）
- [x] 新增位置上报 API（HTTP 批量）
- [x] 新增事件表与索引（delivery_location_events）
- [x] 实现围栏与驻留计算（半径/精度/驻留阈值）
- [x] 完成事件落库与幂等（event_hash 或唯一键）
- [x] 接入可选自动推进开关（默认关闭）
- [x] 通知与日志联动（状态变更时）
- [x] 单测/集成测/Swagger

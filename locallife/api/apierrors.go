...
var (
	...
	ErrSettlementApplicationNotFound = apierr(40455, "settlement application not found")
)
...
var (
	ErrSettlementAccountBankUnsupported = apierr(40062, "account_bank is not supported by the current WeChat bank catalog")
...
var (
	...
	ErrSettlementServiceUnavailable             = apierr(50304, "settlement account service is temporarily unavailable")
	ErrSettlementWechatNoAuth                   = apierr(40364, "the current service provider is not authorized to modify this sub-merchant's settlement account")
	ErrSettlementApplicationQueryNoAuth         = apierr(40365, "the current service provider is not authorized to query this sub-merchant's settlement application")
	ErrSettlementWechatSignError                = apierr(40110, "wechat settlement account signature verification failed: please retry later or contact support")
	ErrSettlementWechatFrequencyLimit           = apierr(42951, "wechat settlement account request rate limit reached: retry later and keep requests under 20 per minute")
	ErrSettlementApplicationQueryFrequencyLimit = apierr(42952, "wechat settlement application query rate limit reached: retry later and keep requests under 100 per second")
	ErrSettlementWechatServiceUnavailable       = apierr(50051, "wechat settlement account service is temporarily unavailable: please retry later")
	ErrSettlementAccountInactive                = apierr(42251, "merchant payment account is not active: wait until WeChat settlement is active before using settlement account operations")
	ErrSettlementWechatInvalidResponse          = apierr(50219, "wechat settlement account response is invalid due to an upstream contract violation")
)

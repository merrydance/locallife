...
func TestResumeRider_ReconcilesStatusFromThreshold(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()

	store := mockdb.NewMockStore(ctrl)
	server := newTestServer(t, store)
	rider := randomRider(102)
	rider.ID = 654
	rider.Status = db.RiderStatusSuspended
	rider.DepositAmount = 20000
	rider.IsOnline = false
	rider.RegionID.Int64 = 12
	operator := db.Operator{ID: 23, UserID: 89, RegionID: 12, RiderDeposit: 30000}
	...
	ctx, recorder := newJSONTestContext(http.MethodPost, "/v1/operator/riders/654/resume", `{"reason":"manual resume"}`)
	ctx.Params = gin.Params{{Key: "id", Value: "654"}}
	ctx.Set(operatorKey, operator)

	server.ResumeRider(ctx)

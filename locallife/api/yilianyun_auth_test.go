...
func TestYilianyunAuthorizationCallbackDoesNotRequireBearerAndStoresEncryptedTokens(t *testing.T) {
	...
	store.EXPECT().AuthorizeYilianyunCloudPrinterTx(gomock.Any(), gomock.Any()).
		DoAndReturn(func(_ context.Context, arg db.AuthorizeYilianyunCloudPrinterTxParams) (db.AuthorizeYilianyunCloudPrinterTxResult, error) {
...
func TestAuthorizeScannedYilianyunPrinterAPI(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name: "OKWithQRKey",
			body: `{"machine_code":"YL-SCAN-001","qr_key":"qr-secret"}`,
			buildStubs: func(store *mockdb.MockStore) {
				expectResolveSingleOwnedMerchant(store, user.ID, merchant)
				store.EXPECT().UpsertCloudPrinterProviderAuthorization(gomock.Any(), gomock.Any()).
					DoAndReturn(func(_ context.Context, arg db.UpsertCloudPrinterProviderAuthorizationParams) (db.CloudPrinterProviderAuthorization, error) {
						require.Equal(t, merchant.ID, arg.MerchantID)
						require.Equal(t, "YL-SCAN-001", arg.MachineCode)
						require.NotContains(t, arg.AccessTokenCiphertext, "scan-access-secret")

...
func TestGetOperatorRealtimeStatsAPI_DoesNotUseApplicationStatusesForRegionRiders(t *testing.T) {
	user, _ := randomUser(t)
	...
	store.EXPECT().
		...
		Return(int64(88), nil)
	store.EXPECT().
		CountRiderApplicationsByStatus(gomock.Any(), db.RiderApplicationStatusSubmitted).
		AnyTimes().
		Return(int64(77), nil)
	...
	require.Equal(t, int32(0), resp.PendingRiderCount)
}

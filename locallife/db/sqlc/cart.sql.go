...
WITH visible_cart_items AS (
    ...
    LEFT JOIN combo_sets cs ON cs.id = ci.combo_id AND cs.deleted_at IS NULL
    WHERE c.user_id = $2
      AND (c.order_type = $3 OR $3 IS NULL)
)
SELECT 
...
func (q *Queries) GetUserCartsSummary(ctx context.Context, arg GetUserCartsSummaryParams) (GetUserCartsSummaryRow, error) {
	row := q.db.QueryRow(ctx, getUserCartsSummary, arg.ExcludePackaging, arg.UserID, arg.OrderType)
...
func (q *Queries) GetUserCartsWithDetails(ctx context.Context, arg GetUserCartsWithDetailsParams) ([]GetUserCartsWithDetailsRow, error) {
	rows, err := q.db.Query(ctx, getUserCartsWithDetails, arg.ExcludePackaging, arg.UserID, arg.OrderType)
	if err != nil {
		return nil, err
	}
	defer rows.Close()

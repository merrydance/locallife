...
// versions:
//   sqlc v1.31.1
// source: baofu_reconciliation.sql
...
const getBaofuDailyReconciliation = `-- name: GetBaofuDailyReconciliation :many
WITH baofu_reconciliation_parts AS (
    ...
    WHERE pso.provider = 'baofu'
      ...
      AND COALESCE(pso.finished_at, pso.created_at) >= $1
      AND COALESCE(pso.finished_at, pso.created_at) <= $2
    ...
    WHERE bwo.status IN ('succeeded', 'processing')
      AND COALESCE(bwo.finished_at, bwo.created_at) >= $1
      AND COALESCE(bwo.finished_at, bwo.created_at) <= $2
    ...
    WHERE epf.provider = 'baofu'
      ...
      AND epf.observed_at >= $1
      AND epf.observed_at <= $2
    ...
    WHERE epc.provider = 'baofu'
      ...
      AND epc.submitted_at >= $1
      AND epc.submitted_at <= $2
...
type GetBaofuDailyReconciliationParams struct {
...
func (q *Queries) GetBaofuDailyReconciliation(ctx context.Context, arg GetBaofuDailyReconciliationParams) ([]GetBaofuDailyReconciliationRow, error) {
	rows, err := q.db.Query(ctx, getBaofuDailyReconciliation, arg.StartAt, arg.EndAt)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []GetBaofuDailyReconciliationRow{}
	for rows.Next() {
		var i GetBaofuDailyReconciliationRow
		if err := rows.Scan(
			&i.Date,
			&i.Provider,
			&i.Channel,
			...
			&i.WithdrawSucceededAmount,
			&i.WithdrawProcessingAmount,
			&i.UnappliedFactCount,
			&i.UnknownCommandCount,
			&i.FeeLedgerMismatchCount,
			&i.HistoricalRetryableFailedRefundCount,

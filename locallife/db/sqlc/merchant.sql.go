...
const clearExpiredMerchantManualOpenStatusOverrides = `-- name: ClearExpiredMerchantManualOpenStatusOverrides :execrows
UPDATE merchants
SET
...
func (q *Queries) ClearExpiredMerchantManualOpenStatusOverrides(ctx context.Context) (int64, error) {
	result, err := q.db.Exec(ctx, clearExpiredMerchantManualOpenStatusOverrides)
	if err != nil {
		return 0, err
	}
...
const getDatabaseLocalClock = `-- name: GetDatabaseLocalClock :one
SELECT
...
type GetDatabaseLocalClockRow struct {
	CurrentYear     int32  `json:"current_year"`
	CurrentMonth    int32  `json:"current_month"`
	CurrentDay      int32  `json:"current_day"`
	LocalTimeMicros int64  `json:"local_time_micros"`
	TimeZone        string `json:"time_zone"`
}

func (q *Queries) GetDatabaseLocalClock(ctx context.Context) (GetDatabaseLocalClockRow, error) {
	row := q.db.QueryRow(ctx, getDatabaseLocalClock)
	var i GetDatabaseLocalClockRow
	err := row.Scan(
		&i.CurrentYear,
		&i.CurrentMonth,
		&i.CurrentDay,
...
func (q *Queries) GetMerchantWithTags(ctx context.Context, id int64) (GetMerchantWithTagsRow, error) {
	...
	err := row.Scan(
		...
		&i.Merchant.ManualOpenStatusUntil,
		&i.Tags,
	)
	return i, err
}
...
const syncMerchantOpenStatusByBusinessHours = `-- name: SyncMerchantOpenStatusByBusinessHours :many
...
func (q *Queries) SyncMerchantOpenStatusByBusinessHours(ctx context.Context) ([]int64, error) {
	rows, err := q.db.Query(ctx, syncMerchantOpenStatusByBusinessHours)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
SET
  ...
  logo_media_asset_id = COALESCE($3, logo_media_asset_id),
...
const updateMerchantAutoOpenByBusinessHours = `-- name: UpdateMerchantAutoOpenByBusinessHours :exec
UPDATE merchants
SET
...
type UpdateMerchantAutoOpenByBusinessHoursParams struct {
	ID                      int64 `json:"id"`
	AutoOpenByBusinessHours bool  `json:"auto_open_by_business_hours"`
}

func (q *Queries) UpdateMerchantAutoOpenByBusinessHours(ctx context.Context, arg UpdateMerchantAutoOpenByBusinessHoursParams) error {
	_, err := q.db.Exec(ctx, updateMerchantAutoOpenByBusinessHours, arg.ID, arg.AutoOpenByBusinessHours)
	return err
}
...
func (q *Queries) UpdateMerchantIsOpen(ctx context.Context, arg UpdateMerchantIsOpenParams) (Merchant, error) {
	row := q.db.QueryRow(ctx, updateMerchantIsOpen,
		arg.ID,
		arg.IsOpen,
		arg.AutoCloseAt,
		arg.ManualOpenStatusUntil,
	)
	var i Merchant
	err := row.Scan(
		&i.ID,

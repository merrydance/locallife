	...
	for attempt := 0; attempt < 5; attempt++ {
		_, err = integrationPool.Exec(context.Background(), `
			TRUNCATE TABLE
				...
				rider_locations,
				riders,
				payment_domain_outbox,
				external_payment_fact_applications,
				external_payment_facts,
				external_payment_commands,
				payment_orders,
				order_items,
				orders,
				carts,
...
func createIntegrationMerchant(t *testing.T, store *db.SQLStore, ownerID, regionID int64) db.Merchant {
	merchant, err := store.CreateMerchant(context.Background(), db.CreateMerchantParams{
		...
		Phone:           "13800138000",
		Address:         fmt.Sprintf("测试地址-%d-%d-%s", ownerID, regionID, util.RandomString(6)),
		Latitude:        pgtype.Numeric{Valid: false},
		Longitude:       pgtype.Numeric{Valid: false},
		Status:          "approved",

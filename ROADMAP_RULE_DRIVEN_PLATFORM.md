...
### 阶段 3：异常订单裁决闭环
- [x] 索赔/异常裁决规则统一到规则引擎（仅行为驱动，无证据依赖，规则模板已补齐，需运营配置生效）
- [x] 责任方裁决与自动执行（判定用户/商户/骑手责任后自动退款/扣款）
- [x] 平台兜底规则（仅当用户、商户、骑手均无明显异常时触发，需运营配置生效）
- [x] 规则驱动淘汰/警示（商户异物/食安高发、骑手餐损/超时高发）
- [x] 追偿单机制与逾期限制接单（商户不影响堂食）
- [x] 无证据裁决数据规范（行为追溯与裁决摘要）
...
- [x] 移除人工复核主流程（仅保留审计旁路，不参与裁决）

### 阶段 4：运营商与平台控制台
- [x] 运营商控制台（规则配置/商户审核/分账复核）
- [x] 平台控制台（风控审计/跨区县运营/异常裁决复核）
- [x] 多区县运营监控大盘
- [x] 规则变更与生效可视化

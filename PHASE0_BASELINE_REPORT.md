...
### 0.1 分账比例配置与执行口径（2026-02-18）
- 平台侧通过管理端（Web/小程序）配置平台佣金比例与运营商佣金比例，后端持久化为可生效分账配置。
- 分账执行阶段按订单维度读取已生效配置，不使用写死比例。
- 佣金计算基数为可分账金额（订单总额扣除骑手侧分成后）。
- 统计口径按真实分账结果聚合（平台收入、运营商收入），不再按固定比例反推。
- 未命中配置时走后端兜底策略（按当前接口/规则约束处理默认值或拒绝）。

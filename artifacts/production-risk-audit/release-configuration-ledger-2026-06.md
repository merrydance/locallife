...
| RC-CUSTOMER-DINE-IN-CHECKOUT | Dine-in checkout | QR/table routing, payment config, session/order/payment workers. | Missing payment recovery or session checkout worker can strand paid sessions. | Include session/payment convergence smoke in release checklist. |

...
| RC-CUSTOMER-TAKEOUT-CHECKOUT | Takeout checkout | Cart/order/payment/refund routes, payment callbacks, timeout workers. | Frontend wrapper drift, disabled timeout/recovery, or unvalidated duplicate order-create behavior can strand checkout state; same-order payment-create concurrency is now covered by focused backend tests. | Include order create/payment pending/callback recovery checklist, and rerun the same-order payment-create tests before payment transaction changes. |

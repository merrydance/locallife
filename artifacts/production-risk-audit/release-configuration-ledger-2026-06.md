...
| RC-CUSTOMER-DINE-IN-CHECKOUT | Dine-in checkout | QR/table routing, payment config, session/order/payment workers, `dine-in-checkout-recovery` scheduler registration, migration `000269`, recovery Prometheus counters, an alert evidence gate that rejects template-only/non-pass/placeholder proof, and a release target evidence gate that rejects static/dry-run/template readiness proof plus local alert-template references. | Missing payment recovery or disabled dine-in checkout recovery scheduler can strand paid sessions; missing migration `000269` can make recovery scans inefficient; missing filled alert or target-smoke evidence can hide repeated recovery failures or disabled release runtime dependencies. | Run `scripts/release_readiness_smoke.sh --target` with disposable fixture IDs, then require `make check-release-readiness-target-evidence evidence=...` plus `check:dine-in-recovery-alert-evidence` before release closure. |
...
| RC-CUSTOMER-RESERVATION | Reservation | Reservation/table APIs, payment/refund callback/recovery, scheduler behavior. | No-show/refund/add-on release requires payment and reservation workers; source-level card now defines the reservation pre-change validation surface; `check:reservation-checkout-addon-recovery-contract` covers the customer Mini Program recovery contract; `check:customer-checkout-provider-e2e-evidence` rejects template-only/non-pass/placeholder provider-device evidence for reservation and reservation add-on release proof. | Include reservation payment/refund smoke before release and use `flows/state-sequencing-customer-reservation-checkout-addon-noshow-2026-06-15.md`; filled real provider/device evidence remains required for full closure. |
...
| RC-CUSTOMER-TAKEOUT-CHECKOUT | Takeout checkout | Cart/order/payment/refund routes, optional `Idempotency-Key` on order create, Mini Program stable order-create key propagation, payment callbacks, timeout workers. | Frontend wrapper drift, disabled timeout/recovery, old clients without the stable key, or provider callback/recovery gaps can strand checkout state; same-order payment-create, backend order-create key replay/concurrency, and current Mini Program key propagation are now covered by focused tests/contracts; `check:customer-checkout-provider-e2e-evidence` rejects template-only/non-pass/placeholder provider-device evidence for release proof. | Include order create/payment pending/callback recovery checklist; rerun the order-create idempotency, same-order payment-create, takeout checkout Mini Program contract, and filled provider/device evidence checks before checkout/payment transaction changes. |
...
| --- | --- | --- | --- |
| RC-BACKLOG-001 | All callback/provider flows | Callback URL, public key, cert serial, terminal/member id, and environment mode must match deployed code. Production Baofu ledger evidence must also bind to filled target-environment release readiness evidence, not only static preflight output. | Use `flows/external-dependency-baofu-provider-evidence-gate-2026-06-15.md`, `locallife/scripts/baofu_provider_evidence_gate.sh`, and the target release readiness smoke before provider-affecting releases; for `--ledger-env production`, require `make check-release-readiness-target-evidence evidence=...` and pass the same file through `--release-target-evidence`. |
| RC-BACKLOG-002 | All scheduler-dependent flows | Many convergence paths depend on scheduler/worker boot, not request code. | Use `flows/release-scheduler-worker-readiness-gate-2026-06-15.md`, `scripts/release_readiness_smoke.sh --target`, and `make check-release-readiness-target-evidence evidence=...`; remaining work is target-environment execution with disposable fixture IDs and filled alert evidence for recovery failure metrics. |
...
This ledger was validated as a documentation artifact only. The target evidence
checker contract was validated locally, but no deployment, environment,
scheduler boot, migration, config, provider credential, or generated-artifact
validation was run.

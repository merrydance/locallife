...
| RC-CUSTOMER-TAKEOUT-CHECKOUT | Takeout checkout | Cart/order/payment/refund routes, optional `Idempotency-Key` on order create, payment callbacks, timeout workers. | Frontend wrapper drift, disabled timeout/recovery, missing Mini Program stable order-create key propagation, or provider callback/recovery gaps can strand checkout state; same-order payment-create and backend order-create key replay/concurrency are now covered by focused backend tests. | Include order create/payment pending/callback recovery checklist; rerun the order-create idempotency and same-order payment-create tests before checkout/payment transaction changes. |

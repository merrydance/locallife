...
| RC-CUSTOMER-DINE-IN-CHECKOUT | Dine-in checkout | QR/table routing, payment config, session/order/payment workers, and `dine-in-checkout-recovery` scheduler registration. | Missing payment recovery or disabled dine-in checkout recovery scheduler can strand paid sessions; missing migration `000269` can make recovery scans inefficient. | Include session/payment convergence smoke, scheduler boot check, and schema version/index check in release checklist. |
...
| RC-BACKLOG-002 | All scheduler-dependent flows | Many convergence paths depend on scheduler/worker boot, not request code. | Use `flows/release-scheduler-worker-readiness-gate-2026-06-15.md`; add a standard scheduler/worker readiness smoke for production releases. |

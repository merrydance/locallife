...
| IR-CUSTOMER-DINE-IN-CHECKOUT | Scan/open session, billing group, dine-in checkout/payment | G3 | Frontend submit guards, backend session/group convergence, payment callback/recovery convergence, paid customer checkout guarded by session owner + active paid dine-in order, and backend scheduler recovery for paid open dine-in sessions through `CloseDiningSessionTx`. | Authz drift in paid-result checkout was fixed in `ad0e609d`; backend post-payment checkout API transient failure is now covered by paid open session recovery. Remaining gap is Mini Program contract/E2E proof for pending checkout context survival and operational alerting for repeated recovery failures. | Treat local session context as draft; backend session/order/payment state owns convergence. |
...
| IR-BACKLOG-004 | Dine-in checkout | Paid-session customer checkout authz drift is fixed and covered; paid session recovery after transient checkout API failure is now implemented with a scheduler, filtered SQL scan, and `CloseDiningSessionTx`. | Keep `flows/state-sequencing-customer-dine-in-checkout-convergence-2026-06-15.md` as the rerun checklist before changing dine-in checkout, shared payment result, or session close behavior; remaining target is Mini Program contract/E2E proof and recovery failure observability. |

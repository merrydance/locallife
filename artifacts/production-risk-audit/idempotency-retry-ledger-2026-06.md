...
| IR-BACKLOG-004 | Dine-in checkout | Paid-session customer checkout authz drift is fixed and covered; paid session recovery after transient checkout API failure is implemented with a scheduler, filtered SQL scan, and `CloseDiningSessionTx`; Mini Program reload/polling contract proof is now in `check:dine-in-checkout-result-reentry-contract`. | Keep `flows/state-sequencing-customer-dine-in-checkout-convergence-2026-06-15.md` as the rerun checklist before changing dine-in checkout, shared payment result, or session close behavior; remaining target is device/E2E proof and recovery failure observability. |

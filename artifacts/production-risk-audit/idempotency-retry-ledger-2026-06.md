...
| IR-CUSTOMER-DINE-IN-CHECKOUT | Scan/open session, billing group, dine-in checkout/payment | G3 | Frontend submit guards, backend session/group convergence, payment callback/recovery convergence, paid customer checkout guarded by session owner + active paid dine-in order. | Authz drift in paid-result checkout was fixed in `ad0e609d`; remaining gap is backend retry/recovery if the post-payment checkout API fails transiently, plus Mini Program contract/E2E proof for pending checkout context survival. | Treat local session context as draft; backend session/order/payment state owns convergence. |
...
| IR-BACKLOG-004 | Dine-in checkout | Paid-session customer checkout authz drift is fixed and covered; paid session recovery after transient checkout API failure remains open. | Keep `flows/state-sequencing-customer-dine-in-checkout-convergence-2026-06-15.md` as the rerun checklist before changing dine-in checkout, shared payment result, or session close behavior; next implementation target is backend retry/recovery for paid open sessions. |

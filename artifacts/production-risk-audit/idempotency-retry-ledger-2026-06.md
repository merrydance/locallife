...
| IR-CUSTOMER-DINE-IN-CHECKOUT | Scan/open session, billing group, dine-in checkout/payment | G3 | Frontend submit guards, backend session/group convergence, payment callback/recovery convergence, paid customer checkout guarded by session owner + active paid dine-in order, backend scheduler recovery for paid open dine-in sessions through `CloseDiningSessionTx`, Mini Program contract proof for result-page reload/polling context survival, and Prometheus recovery counters for list/close outcomes. | Authz drift in paid-result checkout was fixed in `ad0e609d`; backend post-payment checkout API transient failure is covered by paid open session recovery; `check:dine-in-checkout-result-reentry-contract` now locks the Mini Program pending checkout context handoff. Remaining gap is actual device/E2E proof and target-environment alert rules for repeated recovery metric increases. | Treat local session context as draft; backend session/order/payment state owns convergence. |
...
| IR-BACKLOG-004 | Dine-in checkout | Paid-session customer checkout authz drift is fixed and covered; paid session recovery after transient checkout API failure is implemented with a scheduler, filtered SQL scan, `CloseDiningSessionTx`, and Prometheus counters for recovery scan/session outcomes; Mini Program reload/polling contract proof is now in `check:dine-in-checkout-result-reentry-contract`. | Keep `flows/state-sequencing-customer-dine-in-checkout-convergence-2026-06-15.md` as the rerun checklist before changing dine-in checkout, shared payment result, or session close behavior; remaining target is device/E2E proof and target-environment alert rules for recovery failure metrics. |

...
| Scheduler-dependent convergence | Many flows depend on workers/schedulers being deployed and configured. | Release readiness smoke exists in `locallife/cmd/release_readiness_smoke` and `artifacts/production-risk-audit/flows/release-scheduler-worker-readiness-gate-2026-06-15.md`; it now covers `dine-in-checkout-recovery` as part of the scheduler registry. Remaining work is target-environment execution with Redis/provider/fixture flags and operational alerting. |
...
| 2 | Dine-in paid order -> session checkout convergence | Synchronized. Source audit, backend recovery, and Mini Program reload/polling contract proof are now covered. | Covered by `artifacts/production-risk-audit/flows/state-sequencing-customer-dine-in-checkout-convergence-2026-06-15.md`; still needs actual device/E2E proof and recovery-failure observability before treating the release path as fully drilled. |
...
| 7 | Scheduler-dependent convergence release readiness | Synchronized. The release readiness smoke maps production fail-fast, Redis/Asynq, scheduler registration, worker handlers, provider-client readiness, and fixture claimability; the default scheduler registry now includes `dine-in-checkout-recovery`. | Covered by `artifacts/production-risk-audit/flows/release-scheduler-worker-readiness-gate-2026-06-15.md`; the remaining release issue is executing the smoke in the target environment with Redis/provider/fixture checks and adding operational alerting for repeated recovery failures. |

...
| Scheduler-dependent convergence | Many flows depend on workers/schedulers being deployed and configured. | Phase 1 recon is now in `artifacts/production-risk-audit/flows/release-scheduler-worker-readiness-gate-2026-06-15.md`; next implementation should add a release smoke command or script for Redis/Asynq, worker handlers, scheduler registry, and provider-client readiness. |
| Dine-in checkout | Paid order -> session checkout convergence is ledgered but not source-audited in this pass. | Use `artifacts/production-risk-audit/flows/state-sequencing-customer-dine-in-checkout-convergence-2026-06-15.md` as the source-audit execution card before code changes. |
| Customer checkout | Takeout/reservation stale draft -> backend rehydration -> payment callback/recovery deserves end-to-end proof. | Use `artifacts/production-risk-audit/flows/state-sequencing-customer-takeout-checkout-rehydration-payment-2026-06-15.md` for takeout; create a separate reservation card before reservation payment/add-on changes. |
...
| --- | --- | --- | --- |
| 1 | Baofoo provider flows positive callback evidence | Synchronized. Phase 1 recon now maps the evidence gap to callback handlers, fact/application rows, probe commands, and the drift guard. | Covered by `artifacts/production-risk-audit/flows/external-dependency-baofu-provider-evidence-gate-2026-06-15.md`; real positive callback/funds-action evidence remains limited for payment/refund/share/withdrawal provider paths. |
| 2 | Dine-in paid order -> session checkout convergence | Synchronized. Current docs keep this as slice-ledgered/source-audit missing. | Covered by `artifacts/production-risk-audit/flows/state-sequencing-customer-dine-in-checkout-convergence-2026-06-15.md`; still needs source-level audit or E2E proof for paid order -> `checkoutDiningSession`. |
| 3 | Customer stale draft -> backend rehydrate -> payment callback/recovery -> visible status | Synchronized. Current docs keep this as checkout E2E proof missing. | Takeout covered by `artifacts/production-risk-audit/flows/state-sequencing-customer-takeout-checkout-rehydration-payment-2026-06-15.md`; reservation needs a separate card before reservation payment/add-on changes. |
...
| 7 | Scheduler-dependent convergence release readiness | Synchronized. Phase 1 recon now maps the release risk to production fail-fast, Redis/Asynq, scheduler registration, worker handlers, and provider-client readiness. | Covered by `artifacts/production-risk-audit/flows/release-scheduler-worker-readiness-gate-2026-06-15.md`; standard scheduler/worker boot, queue, and recovery smoke checks are still missing from the release gate. |

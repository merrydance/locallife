...
| Dine-in checkout | Historical note: paid order -> session checkout convergence was ledgered but not source-audited in the original pass; later work source-audited it, fixed customer paid checkout authz drift, and added backend paid-open-session recovery. | Use `artifacts/production-risk-audit/flows/state-sequencing-customer-dine-in-checkout-convergence-2026-06-15.md` as the rerun checklist before changing dine-in checkout, shared payment result, or recovery behavior. |

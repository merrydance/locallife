...
| SS-BACKLOG-003 | Dine-in checkout | Paid order -> session checkout convergence has source audit coverage; customer paid checkout authz drift, backend recovery after transient checkout failure, Mini Program reload/polling context contract proof, and recovery failure Prometheus counters are fixed. | Use `flows/state-sequencing-customer-dine-in-checkout-convergence-2026-06-15.md` as the rerun checklist before modifying session checkout; remaining follow-up is actual Mini Program device/E2E proof and target-environment alert rules for recovery failure metrics. |

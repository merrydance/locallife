...
| SS-BACKLOG-003 | Dine-in checkout | Paid order -> session checkout convergence has source audit coverage; customer paid checkout authz drift and backend recovery after transient checkout failure are fixed. | Use `flows/state-sequencing-customer-dine-in-checkout-convergence-2026-06-15.md` as the rerun checklist before modifying session checkout; remaining follow-up is Mini Program contract/E2E proof and recovery failure observability. |

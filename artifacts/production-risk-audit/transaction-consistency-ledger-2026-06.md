...
| TC-CUSTOMER-TAKEOUT-CHECKOUT | Takeout checkout | Order/cart/payment SQL own checkout truth; payment callback applies order paid; `CreatePartnerPaymentTx` locks the order row before checking/inserting a pending Baofu payment order. | Payment result page and frontend snapshots are read-side; `check:takeout-checkout-rehydration-payment-contract` now proves snapshots remain draft until backend pricing, payment-create failure goes to durable order detail, and payment-result re-entry polls backend payment truth; focused Go tests prove same-order payment create concurrency leaves one pending payment order and no second upstream Baofu call after transaction conflict. | Duplicate order create before an order id exists and real provider callback/recovery visibility remain high value. | Backend rehydration remains required before money moves; keep same-order payment creation inside the transaction-owned order lock boundary. |

...
| --- | --- | --- | --- |
| TC-BACKLOG-001 | Dine-in checkout | Paid order to session checkout convergence is source-audited; both customer checkout and backend recovery use `CloseDiningSessionTx`; Mini Program reload/polling context contract proof is in `check:dine-in-checkout-result-reentry-contract`; device/E2E evidence schema is in `check:dine-in-device-e2e-evidence`; recovery failure metrics are exposed through Prometheus. | Keep a filled Mini Program device/E2E evidence file and target-environment alert rules for recovery failure metrics as follow-ups; rerun focused DB/scheduler/weapp contract tests before transaction changes. |

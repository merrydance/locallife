...
| TC-CUSTOMER-TAKEOUT-CHECKOUT | Takeout checkout | Order/cart/payment SQL own checkout truth; payment callback applies order paid. | Payment result page and frontend snapshots are read-side; `check:takeout-checkout-rehydration-payment-contract` now proves snapshots remain draft until backend pricing, payment-create failure goes to durable order detail, and payment-result re-entry polls backend payment truth. | Duplicate order create/payment create transaction proof and real provider callback/recovery visibility remain high value. | Backend rehydration remains required before money moves. |

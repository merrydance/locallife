...
| --- | --- | --- | --- |
| TC-BACKLOG-001 | Dine-in checkout | Paid order to session checkout convergence is source-audited; both customer checkout and backend recovery use `CloseDiningSessionTx`; Mini Program reload/polling context contract proof is in `check:dine-in-checkout-result-reentry-contract`; recovery failure metrics are exposed through Prometheus. | Keep actual Mini Program device/E2E proof and target-environment alert rules for recovery failure metrics as follow-ups; rerun focused DB/scheduler/weapp contract tests before transaction changes. |

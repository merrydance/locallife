...
| ED-CUSTOMER-RESERVATION | Reservation | Payment/refund providers, room/table state, dine-in handoff. | Provider facts terminalize money; reservation/table transactions own local state; source-level card now maps reservation deposit/add-on/refund provider boundaries; `check:reservation-checkout-addon-recovery-contract` covers the customer Mini Program recovery contract; `check:customer-checkout-provider-e2e-evidence` rejects template-only/non-pass/placeholder provider-device evidence for reservation and reservation add-on flows. | Real filled reservation deposit/add-on/refund callback/recovery E2E evidence remains open. | Keep provider money truth in payment domain and use `flows/state-sequencing-customer-reservation-checkout-addon-noshow-2026-06-15.md` before changes; the evidence gate prevents false closure but does not replace provider runs. |
...
| ED-CUSTOMER-TAKEOUT-CHECKOUT | Takeout checkout | WeChat/Baofoo payment, payment result, refunds, delivery-fee calculation. | Backend calculation and provider callback/recovery; `check:customer-checkout-provider-e2e-evidence` rejects template-only/non-pass/placeholder provider-device evidence for takeout checkout. | Filled end-to-end provider callback/recovery evidence belongs to payment domain and remains open until an actual device/provider run is recorded. | Keep checkout pricing and payment terminal truth server-side; do not close provider evidence with local-only contracts. |
...
| --- | --- | --- | --- |
| ED-BACKLOG-001 | Baofoo payment/refund/share/withdrawal | Baofu source matrix records remaining positive callback evidence gaps for real provider flows; a read-only evidence gate wrapper now exists but does not create provider evidence by itself. Production ledger evidence also depends on filled target-environment release readiness evidence, not only the wrapper's static preflight. | Use `flows/external-dependency-baofu-provider-evidence-gate-2026-06-15.md` and `locallife/scripts/baofu_provider_evidence_gate.sh`; for `--ledger-env production`, first pass `make check-release-readiness-target-evidence evidence=...` and pass the same file with `--release-target-evidence`; do not change provider semantics without updating Baofu domain evidence. |

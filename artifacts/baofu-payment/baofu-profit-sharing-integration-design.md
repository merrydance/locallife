...
- `sharingMerId` 按宝付二级户接收方标识建模，首版优先绑定开户返回的 `contractNo` 或宝付提供的等价二级户号；无论具体字段名是什么，`openid` 本身不应作为首版分账接收方主键。
...
| 问题 | 当前结论 | 实施口径 |
| --- | --- | --- |
| `sharingMerId` 精确定义 | 可以确定它不是 `openid`，也不是微信报备返回的 `subMchId`。协议支付确认分账示例使用 `CM...`、`CP...` 这类宝财通二级户号；账户查询把 `contractNo` 定义为客户账户号；聚合商户报备把 `bctMerId` 定义为宝财通二级用户客户号。 | 按“宝付平台二级户/宝财通二级户接收方标识”落库。实现上保存开户返回的 `contractNo`，同时预留 `sharing_mer_id` 字段承接宝付生产环境最终要求；不得使用微信 `subMchId` 或 `openid` 作为分账接收方。 |
...
5. 开户成功后保存 `contractNo`，作为本地业务主体与宝付账户的绑定标识。
...
- 确认 `sharingMerId` 与宝财通开户返回 `contractNo` 是否同值；如不同，补充映射字段和同步流程。

...
- 骑手要直接拿配送费全额分账，必须在宝付侧存在可收款主体。宝财通个人账户开户成功后返回的二级商户号必须同步到本地 `sharing_mer_id`，分账到账后先进入在途余额，结算后进入可用余额，再提现到绑定银行卡。
...
- 骑手工作台、接单大厅和配送费结算页在结算账户不可用时展示：`结算账户未开通，暂不能接收配送费分账订单`。
- 商户 Flutter App 设置页新增 `结算账户` 说明页，覆盖 `宝付支付开通`、`微信渠道报备`、`结算账户可用`、`支付手续费`、`待结算金额`、`可提现金额`、`提现中`、`提现成功`、`提现退回`。

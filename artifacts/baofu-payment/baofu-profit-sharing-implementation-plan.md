...
- [x] **Step 1: Split endpoint profiles**
...
- [x] **Step 2: Model public envelope separately from bizContent**
...
- [x] **Step 3: Generate appendix enums and category allowlists**
...
- [x] **Step 4: Validate anti-drift gates**
...
- [x] **Step 1: Model confirmed report boundary**
...
- [x] **Step 2: Persist report lifecycle**
...
- [x] **Step 3: Integrate readiness and product copy**
...
### 2026-05-04 Sandbox Smoke Follow-up - Public Envelope Wire Format

- Rerunning smoke on `6bec7b1f` still returned `商户号不能为null` even though the `PaymentQueryRequest` DTO carried `merId/terId`. That ruled out the DTO field-population hypothesis and shifted the root cause to public-envelope wire serialization.
- Rechecked Baofoo 聚合支付「接口请求入口」 and Java demo. The official request table marks `bizContent` as type `S` with JSON content, and the demo sets `PostMasterEntity.bizContent` to `JSON.toJSONString(Biz_Content)`; `signStr` is generated by `SHA256withRSA` and `FormatUtil.byte2Hex(signature.sign())`.
- Corrected the public envelope client to serialize `bizContent` as a JSON string rather than an embedded object, while keeping response parsing tolerant of both string and object forms. Corrected RSA public-envelope signature encoding to hex instead of base64.
- This is still C3/C4 bridge work until the fixed commit is deployed and `order_query` returns an order-not-found/config-specific business response rather than a null-merchant parsing error.


### 2026-05-04 Sandbox Smoke Follow-up - Public Envelope Form Transport
...
- Rerunning the diagnostic smoke after deploying `b4961434` parsed `dataContent` successfully: `order_query` returned `resultCode=SUCCESS` and `txnState=ABNORMAL` for the synthetic order. This grants sandbox evidence for the `order_query` public envelope + response business-content shape, but not for a real paid-order query, callback, local fact persistence, or payment application.

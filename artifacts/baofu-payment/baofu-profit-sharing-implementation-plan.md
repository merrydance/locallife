...
- Residual risk: this slice wires HTTP API main-business single-payment creation. Combined payment Baofu support, response signature verification/digital envelope completeness, refund callback/query recovery, full error-code tables, and real Baofu test-address evidence remain open for later remediation/sandbox tasks.
...
- Preserved the open risks explicitly: union-gw official envelope/数字信封/响应验签, combined-payment strategy, refund callback/query recovery, full official error-code tables, and real sandbox evidence.
...
- Residual risk: this slice wires aggregate payment/share client runtime only. BaoCaiTong account union-gw envelope/response verification, combined-payment strategy, refund callback/query recovery, and sandbox evidence remain open.

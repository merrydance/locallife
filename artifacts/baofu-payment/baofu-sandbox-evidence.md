...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 2026-05-05 | sandbox | `https://mch-juhe.baofoo.com/mch-service/api` | generated by smoke script | personal account + storefront short name | `CP61***2938` | reached sandbox; provider rejected with `REPORT_NAME_NOT_CUSTOMER_NAME` | - | `099aa07d` | Negative merchant-report evidence. For `IDENTITY_CARD` report under a personal BaoCaiTong account, Baofoo requires the reported `merchant_name` to match the BaoCaiTong account customer name. Retry with `merchant_name` equal to the account holder name and storefront only in `merchant_shortname`, or use a business/individual-business BaoCaiTong account plus business license report data. |

...
| 2026-05-05 | sandbox | `https://mch-juhe.baofoo.com/api` | `BAOFU_UO_20260505113416` | requested `4000***0573`; effective wire `omitted_by_client` | 100 | no | no | no | next fix | Repo smoke proved sandbox client omits `subMchId` on the wire. Baofoo returned a business payload shape where `chlRetParam.order_id` is numeric; local DTO still required string and the unmarshal failure was reported as upstream_code `SUCCESS`. Fixed next by accepting numeric/string `order_id` and classifying business payload unmarshal failures as `INVALID_DATA_CONTENT`, not `SUCCESS`. |

...
| 规则、运费、高峰时段 | 完整 | 运费/天气/高峰/骑手押金规则有消费点 | 已承接 rules/delivery-fee/timeslot | G2 | 可用，区域押金已按区域配置优先接入主链，运费类仍需持续验证下游计算生效 |
...
| operators/me 规则代理 | 完整 | 版本、发布、回滚、禁用、命中查询具备区域约束 | 产品边界待定，不默认承接 | G2 | 通用规则工作台能力，不等于当前轻量运营规则配置 |
...
主流程接入：已接入运费计算、天气系数和骑手押金门槛。骑手押金运行时读取顺序已收敛为 `region_rule_configs.rider_deposit` -> 全局平台配置 `rider_deposit_fen` -> 系统默认 `DefaultRiderDepositThresholdFen`；旧 `operator.rider_deposit` 和运营商 scope 平台配置不再作为主链读取来源。

关闭证据：2026-04-29 已更新 `locallife/db/sqlc/rider_status_helpers.go`、`locallife/api/operator_rules.go`、`locallife/logic/rider_workbench.go` 及对应测试。验证覆盖区域优先、平台兜底、系统默认、区域更新后骑手状态重算、API 查询和支付查询测试对齐；本次未改 SQL 源，不需要 `make sqlc`。

验证重点：规则变更后对运费预览/下单结算的实际影响需要集成级验证。
...
weapp 处理方向：operator weapp 不承接分账金额、receiver 删除、补差创建/退回/取消。若未来需要资金诊断视图，应作为只读审计信息，不开放运营商手工操作。
...
- 追偿争议与追偿详情：系统自动裁决，不是运营商待办；只读观察是否需要承接待定。
- 微信支付投诉记录：来源于微信支付投诉通知/同步，不是默认运营商客服处理；只读监管是否需要承接待定。
- 分账配置：平台级配置，只读展示可选，不做运营商配置入口。
- operators/me 规则代理：通用规则工作台能力，是否开放给运营商需另定产品边界。

承接质量问题：

- 商户/骑手页面已不属于当前运营商能力承接范围；现有入口应收口或隐藏，若保留只读诊断也不得展示后端未返回字段。

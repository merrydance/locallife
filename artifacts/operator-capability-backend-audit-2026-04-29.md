...
闭环判断：完整。读侧从 `deliveries`、`orders`、`merchants` 聚合待接单代取单；调度器和 worker 会基于超时扫描写入运营商通知。
...
- 规则、运费、高峰时段：OP-CAP-07 后端验证和消费矩阵已完成；operator weapp 继续承接轻量区域规则，不接入通用规则代理。
- Admin 运营商治理：OP-CAP-10 后台治理审计已完成；不纳入 operator weapp，后续 web/admin 承接需另建后台审批/状态/receiver ViewState。
...
- 商户/骑手页面已收口为只读档案语义，dashboard 已移除“待办审批”聚合，不再把商户/骑手展示为运营商待处理事项。
- `weapp/miniprogram/api/operator-basic-management.ts` 中不存在的 `GET/PATCH /v1/operators/me` wrapper 和依赖 helper 已清理。
- 派单监控 dispatch hall 已有摘要、列表、权限和超时标记契约；OP-CAP-08 已补充 scheduler/worker 告警链路验证。

## 后续验证建议

本轮运营商能力闭环文档已完成。以下建议作为持续治理规则保留，不代表当前仍有未关闭任务：

- 后端：后续改动 G2/G3 运营商链路时，继续补单元测试或集成测试证据，不用默认大跑全量。
- weapp：新增高风险操作页前，先完成 ViewState 和失败态设计，避免按 API 平铺按钮。
- 文档：新增任务完成后继续回填“源码闭环证据 + 验证命令 + 残余风险”。
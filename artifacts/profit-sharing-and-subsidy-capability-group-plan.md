...
状态备注（2026-04-26）：本文件保留为能力组全景计划，不是当前 MVP 的执行基线。当前实现顺序只继续推进分账剩余异步链路；补差保持 deferred/non-MVP，除已落地能力的文档同步和缺陷修正外，不作为当前实现重点。
...
- 项目内补差 create、return、cancel 的能力组闭环实现。
- 因历史上“补差与分账组”并列而把补差重新拉回当前实现重点。
...
- 直连支付链路重构
- 对现有业务分润比例做产品策略调整
- 补差 create、return、cancel 的能力组闭环与对象级授权整改；当前仅保留已落地命令审计层与缺陷修正。

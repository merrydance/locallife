...
- `isDuplicateKeyError` uses fragile error-string matching and slices the first nine characters after checking only non-empty length. A short unexpected error string can panic; a normal driver duplicate error may also fail to map to the intended conflict.
...
- Replace duplicate-error string slicing with typed database error classification.
...
- Test-proof gaps checked: existing tests cover invite pending semantics, RBAC role selection, group draft/review/join basics, group OCR ownership/worker, and terminal transaction conflicts. Missing proof remains for disabled rejoin contract, atomic staff/coarse-role propagation, every pending-role consumer, invite revocation/disabled-merchant bind, concurrent group OCR JSON writes, direct document ownership validation, submit completeness, active draft uniqueness, join audit failure, cancel/approve race, and duplicate-error classification.

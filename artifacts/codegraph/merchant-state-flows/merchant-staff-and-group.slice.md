...
6. Bind route resolves the merchant by bind code, verifies expiry, rejects existing active/pending membership rows, and lets an existing disabled row re-enter through `AddMerchantStaffTx` as `merchant_staff(role='pending', status='active')`. It deliberately does not grant the global `merchant_staff` user role yet.
   Evidence: `locallife/api/server.go:722`, `locallife/api/staff.go:391`, `locallife/api/staff.go:397`, `locallife/api/staff.go:411`, `locallife/db/sqlc/tx_merchant_staff.go:46`, `locallife/db/sqlc/tx_merchant_staff.go:71`.
...
- `locallife/api/staff_test.go` proves invite bind creates pending staff through `AddMerchantStaffTx` without granting the coarse merchant-staff role, disabled invite re-application returns pending without direct role grants, owner direct-add reactivates a disabled row with the confirmed assigned role, and add/update/delete handlers use the atomic staff-role tx entrypoints instead of split writes.
...
- Invite-code revoke/rotation behavior, including immediate invalidation of old codes.
...
- Failure/retry branches checked: invite-code reuse, fixed disabled staff pending re-application/owner-confirmed reactivation, fixed transaction-owned staff/user-role propagation, fixed group draft duplicate get-or-create, OCR stale asset guard, fixed group submit backend completeness enforcement, pending-only join duplicate constraint, repeated rejected/cancelled terminal history, fixed cancel-after-approve conflict handling, fixed join create/reject/cancel transaction-owned audit writes, typed duplicate-key classification, and Mini Program stable-code `40980` duplicate-pending recovery.

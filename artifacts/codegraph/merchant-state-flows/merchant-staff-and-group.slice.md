...
- Fixed 2026-06-10: a disabled/removed staff member may re-apply through a valid invite code, but only into `pending`; owner direct-add may reactivate the same disabled membership row only with a confirmed real role before operational access is restored.
- Fixed 2026-06-10: staff invite codes support explicit revoke/rotate, old codes become invalid immediately through the single-current-code `merchants.bind_code` lookup boundary, and stored non-empty bind codes on non-deleted merchants are globally unique at the database boundary.
...
1. Merchant staff page loads backend staff truth, gets or generates the current invite code, explicitly rotates/revokes invite codes, updates roles, and soft-removes staff.
   Evidence: `weapp/miniprogram/pages/merchant/staff/index.ts:236`, `weapp/miniprogram/pages/merchant/staff/index.ts:297`, `weapp/miniprogram/pages/merchant/staff/index.ts:329`, `weapp/miniprogram/pages/merchant/staff/index.ts:404`, `weapp/miniprogram/pages/merchant/staff/index.ts:457`.

2. Staff wrappers call `GET /v1/merchant/staff`, `POST /invite-code`, `POST /invite-code/rotate`, `POST /invite-code/revoke`, `PATCH /:id/role`, and `DELETE /:id`.
   Evidence: `weapp/miniprogram/pages/merchant/_api/merchant-staff.ts:62`, `weapp/miniprogram/pages/merchant/_api/merchant-staff.ts:72`, `weapp/miniprogram/pages/merchant/_api/merchant-staff.ts:79`, `weapp/miniprogram/pages/merchant/_api/merchant-staff.ts:86`, `weapp/miniprogram/pages/merchant/_api/merchant-staff.ts:93`, `weapp/miniprogram/pages/merchant/_api/merchant-staff.ts:101`.

3. Staff list and invite-code routes allow owner/manager; direct add, role update, and remove routes are owner-only.
   Evidence: `locallife/api/server.go:762`, `locallife/api/server.go:766`, `locallife/api/server.go:767`, `locallife/api/server.go:768`, `locallife/api/server.go:772`.

4. Invite-code get-or-generate reuses an unexpired `merchants.bind_code`; when no active code is visible in the request snapshot, it writes through `CreateMerchantBindCodeWhenInactive` so a concurrent popup open cannot overwrite a just-created active code and instead re-reads the current code. Explicit rotate creates a distinct random 32-character code with a 24-hour expiry; explicit revoke clears the stored code and expiry. Migration `000263` enforces global uniqueness for stored non-empty bind codes on non-deleted merchants, and the API retries on `23505` collisions before returning failure.
   Evidence: `locallife/api/staff_invite.go:34`, `locallife/api/staff_invite.go:41`, `locallife/api/staff_invite.go:49`, `locallife/api/staff_invite.go:63`, `locallife/api/staff_invite.go:74`, `locallife/api/staff_invite.go:88`, `locallife/api/staff_invite.go:95`, `locallife/api/staff_invite.go:108`, `locallife/api/staff_invite.go:120`, `locallife/api/staff_invite.go:143`, `locallife/api/staff_invite.go:155`, `locallife/api/staff_invite.go:164`, `locallife/db/query/merchant.sql:208`, `locallife/db/migration/000263_harden_merchant_staff_invite_code_uniqueness.up.sql:1`, `locallife/db/migration/000263_harden_merchant_staff_invite_code_uniqueness.up.sql:17`.
...
6. Bind route resolves the merchant by bind code, verifies expiry, rejects existing active/pending membership rows, and lets an existing disabled row re-enter through `AddMerchantStaffTx` as `merchant_staff(role='pending', status='active')`. It deliberately does not grant the global `merchant_staff` user role yet.
   Evidence: `locallife/api/server.go:759`, `locallife/api/staff.go:306`, `locallife/api/staff.go:316`, `locallife/api/staff.go:327`, `locallife/api/staff.go:347`, `locallife/db/sqlc/tx_merchant_staff.go:46`, `locallife/db/sqlc/tx_merchant_staff.go:71`.

7. Owner role assignment uses `AssignMerchantStaffRoleTx`: it locks the staff row, verifies merchant ownership and non-owner mutation, changes `merchant_staff.role`, keeps membership active, and creates/reactivates `user_roles(role='merchant_staff')` in the same transaction. Owner direct-add of a disabled historical staff row also reuses `AddMerchantStaffTx` to reactivate the same row with the owner-confirmed assigned role.
   Evidence: `locallife/api/staff.go:177`, `locallife/api/staff.go:201`, `locallife/db/sqlc/tx_merchant_staff.go:71`, `locallife/db/sqlc/tx_merchant_staff.go:75`, `locallife/db/sqlc/tx_merchant_staff.go:91`, `locallife/db/sqlc/tx_merchant_staff.go:97`, `locallife/db/query/user_role.sql:11`.
...
8. Owner removal uses `RemoveMerchantStaffTx`: it locks the staff row, verifies merchant ownership and non-owner mutation, soft-deletes the membership, then disables the coarse user role only when no assigned active merchant role remains.
   Evidence: `locallife/api/staff.go:246`, `locallife/api/staff.go:264`, `locallife/db/sqlc/tx_merchant_staff.go:110`, `locallife/db/sqlc/tx_merchant_staff.go:114`, `locallife/db/sqlc/tx_merchant_staff.go:125`, `locallife/db/sqlc/tx_merchant_staff.go:151`, `locallife/db/query/merchant_staff.sql:53`.
...
- Fixed 2026-06-10: disabled/removed staff rejoin is allowed only by re-applying with a valid invite into pending status, followed by owner role confirmation. Runtime now reuses the disabled `merchant_staff` row, keeps invite re-application from granting coarse `user_roles.merchant_staff`, reactivates the coarse role only for owner-confirmed assigned-role direct add, and still treats active/pending duplicate bind/direct-add attempts as conflicts.
- Fixed 2026-06-10: invite codes are explicitly revocable/rotatable. `POST /v1/merchant/staff/invite-code` keeps popup open/retry stable by reusing an active code, and cold concurrent opens use a conditional DB create plus current-code re-read instead of overwriting each other. Rotate/revoke mutate the single current `merchants.bind_code`; `POST /v1/bind-merchant` only accepts that current code, so old codes fail immediately after rotation or revocation. Migration `000263` cleans historical duplicate stored bind codes on non-deleted merchants, adds `merchants_active_bind_code_uidx`, and API generation retries on unique collisions.
...
- `merchants.bind_code`, `merchants.bind_code_expires_at`: current staff invite bearer credential. Fixed 2026-06-10: rotate overwrites this single credential, revoke clears it, bind lookup only accepts the current stored value, and `merchants_active_bind_code_uidx` enforces global uniqueness for stored non-empty credentials on non-deleted merchants.
...
- Invite get-or-generate intentionally reuses an existing unexpired code so opening the invite popup does not rotate credentials. Fixed 2026-06-10: cold concurrent get-or-generate requests use conditional persistence and re-read the current active code rather than returning a code that a later popup immediately invalidates; explicit rotate/revoke actions invalidate old codes immediately by overwriting or clearing `merchants.bind_code`, Mini Program actions are guarded against duplicate submits, and generated-code collision with another stored merchant bind code is handled by database uniqueness plus retry.
- Binding the same active or pending membership returns conflict. Fixed 2026-06-10: binding a disabled/removed membership with a valid invite reactivates the same row as a pending re-application that still requires owner role confirmation; owner direct-add can reactivate a disabled row with a confirmed assigned role.
...
- Staff page uses backend list truth, uses get-or-generate for popup open/retry, exposes copy/rotate/revoke/regenerate invite states, applies a local optimistic patch after role/remove success, then reloads.
...
- `locallife/api/staff_test.go` proves invite bind creates pending staff through `AddMerchantStaffTx` without granting the coarse merchant-staff role, disabled invite re-application returns pending without direct role grants, owner direct-add reactivates a disabled row with the confirmed assigned role, get-or-generate reuses an active invite code without rewriting it, stale-snapshot cold get-or-generate re-reads and returns the concurrently created current code, rotate returns a distinct new code, rotate retries on database bind-code collision, revoke clears the current code, revoked-code bind is rejected before staff tx mutation, and add/update/delete handlers use the atomic staff-role tx entrypoints instead of split writes.
- `locallife/db/sqlc/merchant_staff_test.go` proves pending staff is excluded from role-agnostic access/counts, assigned staff grants/reactivates coarse `user_roles.merchant_staff`, pending staff does not grant it, active/pending duplicate add attempts return `ErrMerchantStaffAlreadyExists`, disabled-row reactivation as pending keeps the coarse role disabled, disabled-row reactivation with an assigned role reactivates it, removal disables it only when no assigned active staff remains, pending-only remaining staff disables it, and owner staff cannot be mutated through staff tx helpers.
- `locallife/db/sqlc/merchant_test.go` proves `GetMerchantByOwner` excludes pending staff, rotate invalidates the old `merchants.bind_code`, conditional get-or-generate persistence does not overwrite an active code, revoke clears the old code from lookup, and duplicate stored bind-code writes are rejected; `locallife/db/sqlc/merchant_staff_invite_code_migration_test.go` proves migration `000263` cleans historical duplicate stored codes and preserves the unique-index guard; `locallife/logic/service_support_test.go` proves logic `resolveMerchantForUser` does not fall back to a pending-only merchant association.
...
- Fixed 2026-06-10: disabled/removed staff may re-apply through a valid invite into pending, and owner direct-add may reactivate the disabled row only with an owner-confirmed assigned role. Invite re-application does not grant the coarse role; active/pending duplicate add/bind attempts still conflict.
...
- Fixed 2026-06-10: staff invite codes support revoke/rotate on demand; old codes are invalid immediately after rotation/revocation through the single-current-code lookup boundary, and stored invite-code global uniqueness is enforced by migration `000263` plus API collision retry.
...
- Request branches checked: staff list/invite get-or-generate/invite rotate/invite revoke/bind/update-role/remove, group application get-or-create/update/delete/submit/review aliases, group OCR create/poll, group document binding/delete, group member/role routes, merchant group join create/cancel/reject/approve/list, and group audit-log writes.
- Backend state branches checked: reusable staff invite code, invite-code rotation, invite-code revocation, pending/active/disabled `merchant_staff`, coarse `user_roles`, staff role update/removal, group application draft/submitted/approved/rejected states, group OCR JSON writeback, group creation, group membership roles, join request pending/approved/rejected/cancelled states, merchant `group_id/brand_id` affiliation, and audit logs.
...
- Failure/retry branches checked: invite-code reuse, cold get-or-generate stale-snapshot recovery, fixed invite-code rotate/revoke invalidation, invite-code unique-collision retry, fixed disabled staff pending re-application/owner-confirmed reactivation, fixed transaction-owned staff/user-role propagation, fixed group draft duplicate get-or-create, OCR stale asset guard, fixed group submit backend completeness enforcement, pending-only join duplicate constraint, repeated rejected/cancelled terminal history, fixed cancel-after-approve conflict handling, fixed join create/reject/cancel transaction-owned audit writes, typed duplicate-key classification, and Mini Program stable-code `40980` duplicate-pending recovery.
...
- Zombie/unreachable branches checked: staff invite revoke/rotation UI/API is no longer missing after the 2026-06-10 fix; group review has two admin route aliases; group join backend cancel path is still not represented as a Mini Program applicant cancellation control.
- Test-proof gaps checked: existing tests cover invite pending semantics, invite-code active reuse, cold stale-snapshot get-or-generate recovery, rotate/revoke invalidation, invite-code unique-collision retry and migration hardening, disabled staff pending re-application and owner-confirmed reactivation, pending role-aware access helpers, `GetMerchantByOwner`/logic fallback filtering, atomic staff/coarse-role propagation, typed duplicate-key classification, RBAC role selection, representative real kitchen/inventory route denial for pending staff, group draft/review/join basics, group active-draft uniqueness/reset idempotency, direct group license media validation, group OCR ownership/worker, group OCR JSON patch merging, group submit minimum-document completeness, join create/reject/cancel transaction paths, stable joined-merchant create conflict mapping, pending-only join uniqueness, repeated terminal join history, applicant-history readback, Mini Program join recovery, and terminal transaction conflicts.

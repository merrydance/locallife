...
Status: merchant-state flow slice created; owner-only approval lookup fixed 2026-06-07; read-only `/v1/merchants/applications/me` compatibility route verified 2026-06-11; mutating `GET /v1/merchant/application` submitted-reset race fixed 2026-06-11
...
- Explicit editing of a `submitted` application through write paths can still reset the application to draft while a queued review run exists. The async review worker later refuses non-submitted applications; that avoids accidental approval after an intentional edit, but can leave the queued/processing review run failed/retrying rather than cleanly cancelled.
...
- Explicit edit/reset of a submitted application while a review run exists: queued run should be cancelled/skipped cleanly or made observable as superseded.
...
- Cancel or mark superseded queued review runs when an explicit edit/reset moves an application back to draft, or make the worker treat non-submitted status as a terminal skip with review-run state update.
...
- Async branches checked: OCR asynq worker and provider retry, frontend OCR terminal polling plus application rehydration, stale OCR cleanup scheduler, onboarding review asynq worker, sync fallback when queue unavailable, credential restore notification worker/path, read-path no-reset behavior for submitted applications, and missing stale queued-run scanner after explicit edit/reset.
...
- Test-proof gaps checked: existing tests cover application submit branches, read-only current-application compatibility route behavior, `GET /v1/merchant/application` submitted no-reset behavior, explicit edit reset behavior, OCR owner/media auth, async OCR response/writeback, review worker approval, credential activation, approval transaction basics, approval image copy, staff-merchant collision avoidance, and application/user owner mismatch rejection. Missing proof remains for approved edit/reset product semantics, queued-run reset/cancel semantics after explicit edit/reset, stale pending OCR cleanup, submit retry with existing review run, and failed-validation consent audit semantics.

...
- Async branches checked: OCR asynq worker and provider retry, frontend OCR terminal polling plus application rehydration, stale OCR cleanup scheduler, onboarding review asynq worker, sync fallback when queue unavailable, credential restore notification worker/path, read-path no-reset behavior for submitted applications, explicit edit/reset superseded-review cancellation with worker race skip/summary refresh, and submitted retry reuse of queued/processing onboarding review runs.
- Failure/retry branches checked: duplicate frontend save/submit/upload guards, stale OCR result after asset change, stale pending/processing OCR cleanup, submit validation failure after submit-attempt agreement consent audit, submit mutating OCR JSON before later failure, async enqueue fallback, explicit submitted/approved edit causing draft reset, review summary/credential activation failure after approval transaction, submitted retry with existing queued/processing review run, duplicate queued-task enqueue, and fallback using the same existing queued run id.

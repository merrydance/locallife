...
Fixed 2026-06-08: voucher quantity update now returns a stable 400 before SQL write when `total_quantity < claimed_quantity`; discount-rule and delivery-promotion updates now merge existing rule values with partial PATCH fields and apply create-level value validation before writing; discount `/applicable` plus `/best` route reachability is locked by a focused API route test; voucher and discount management list `total` now returns the full matched backend count. Fixed 2026-06-09: order preview now passes `order_type` into the typed cart lookup, takeout cart preview/order preview/direct order creation now have focused parity proof for subtotal, merchant discount, voucher amount, delivery fee discount, and persisted total under the same marketing stack, and delivery promotions now have DB constraints for positive amounts, `discount_amount <= min_order_amount`, and `valid_until > valid_from`. Fixed 2026-06-10: disabled, soft-deleted, not-yet-started, or expired voucher templates are blocked for future list/preview/order use, including already issued unused vouchers, while existing orders and cancellation rollback remain snapshot-based. Fixed 2026-06-10: recharge-rule delete now deactivates the rule instead of physically deleting it, so historical membership recharge transactions keep their `recharge_rule_id` provenance while active/matching readers exclude the deactivated rule.
...
12. Fixed 2026-06-10: user available voucher lists now filter current template truth (`deleted_at IS NULL`, `is_active=true`, and template validity window). `GetUserVoucher` still returns issued voucher rows but carries `voucher_template_block_reason`; `ValidateVoucher` and promotion-engine voucher trials fail closed for deleted, inactive, not-started, or expired templates.
    Evidence: `locallife/db/query/voucher.sql:126`, `locallife/db/query/voucher.sql:159`, `locallife/db/query/voucher.sql:174`, `locallife/logic/order_pricing.go:71`, `locallife/logic/order_pricing.go:88`, `locallife/logic/promotion_engine.go:251`.

13. Fixed 2026-06-10: order creation and direct voucher use recheck template truth inside the transaction before marking a user voucher used, so a template disabled, soft-deleted, not-yet-started, or expired between preview and order creation cannot be consumed. Order cancellation still rolls back from persisted order/user-voucher snapshot state and does not retroactively revalidate the current template.
    Evidence: `locallife/db/sqlc/tx_create_order.go:110`, `locallife/db/sqlc/tx_create_order.go:173`, `locallife/db/sqlc/tx_voucher.go:144`, `locallife/db/sqlc/tx_order_status.go:260`, `locallife/db/sqlc/tx_order_status.go:267`, `locallife/db/sqlc/tx_order_status.go:285`.
...
17. Fixed 2026-06-10: recharge create validates time, update merges existing and incoming times before validating, and delete now deactivates the row by setting `is_active=false` instead of physically removing it.
    Evidence: `locallife/logic/recharge_rule.go:43`, `locallife/logic/recharge_rule.go:44`, `locallife/logic/recharge_rule.go:105`, `locallife/logic/recharge_rule.go:122`, `locallife/logic/recharge_rule.go:130`, `locallife/logic/recharge_rule.go:176`, `locallife/db/query/membership.sql:131`.
...
- `membership_transactions.recharge_rule_id` still has `ON DELETE SET NULL` as schema protection, but runtime recharge-rule delete now avoids that fallback by deactivating the rule so transaction provenance remains directly auditable.
- Voucher templates are soft-deleted, discount rules are soft-deleted, recharge-rule delete is now business deactivation, and delivery promotions are currently physically deleted.
- User vouchers are durable independent rows after claim. Fixed 2026-06-10: template disable/delete/not-current blocks future list/preview/order use even for already issued but unused vouchers; orders already created are not retroactively changed.
...
- `recharge_rules`: durable rows for recharge bonus rules; owns exact recharge amount, bonus amount, active flag, and valid period. Delete semantics deactivate the row instead of physically removing it.
...
- Voucher API status/claim/update tests and `tx_voucher_test.go` cover claim, duplicate claim, stock, inactive/expired, use, concurrent claim behavior, rejecting `total_quantity < claimed_quantity` before SQL update, and `TestUseVoucherTxRejectsUnavailableTemplate` for inactive/deleted/not-started/expired templates.
- `voucher_test.go` covers `TestGetUserVoucherReturnsTemplateBlockReason`, `TestListUserAvailableVouchersFiltersInactiveAndDeletedTemplates`, and `TestListUserAvailableVouchersForMerchantFiltersInactiveAndDeletedTemplates`.
- `tx_create_order_test.go` and `tx_order_status_test.go` cover using vouchers during order creation, `TestCreateOrderTxRejectsUnavailableVoucherTemplate` for inactive/deleted/not-started/expired templates, and rolling vouchers back on cancellation.
- Recharge-rule logic/API/sqlc tests cover create/update/delete/list authz, update invalid dates, and delete-as-deactivate semantics with historical transaction provenance.
...
- Order calculation and promotion engine tests cover voucher trials, merchant discount stacking, and payment assessment interactions. `TestValidateVoucher`, `TestIsVoucherValid`, and `TestOrderServiceCreateOrderMapsVoucherTemplateUnavailable` cover blocked voucher-template propagation into preview/direct order paths; `TestOrderServiceCreateOrder_MarketingTotalsMatchCartAndOrderPreview` covers order preview's typed cart lookup plus cart preview, order preview, and direct order creation parity for a combined merchant discount, voucher, delivery fee, and delivery-fee discount stack.

Missing high-value tests:

- Recharge-rule deactivate/disable semantics are now proof-covered so delete no longer removes rule provenance from historical recharge references.
...
- Fixed 2026-06-10: voucher template disable/delete/not-current blocks future list/preview/order use, including already issued but unused vouchers; already-created orders and cancellation rollback are not retroactively revalidated.
...
- Fixed 2026-06-10: recharge-rule delete deactivates the row instead of physically deleting it. `ListActiveRechargeRules` and `GetMatchingRechargeRule` exclude the inactive rule, while `GetRechargeRule` and historical `membership_transactions.recharge_rule_id` can still show the rule used by past recharges.
...
- Backend state branches checked: voucher templates and issued `user_vouchers`, discount rules, delivery promotions, recharge rules, membership transaction rule link, order persisted pricing fields, active/expired/soft-deleted states, issued-voucher usability after template change, and route ordering for static discount endpoints. Voucher-template blocking is fixed for future list/preview/order use, and recharge-rule delete now deactivates instead of physically deleting historical rule truth.
...
- Failure/retry branches checked: local submit/toggle guards, no CRUD idempotency/version, full-count backend list total with frontend lookahead fallback, voucher update quantity below claimed quantity, recharge-rule delete with historical transactions, and delivery-promotion direct SQL/future writers protected by DB constraints. Recharge-rule delete now preserves historical rule provenance by deactivating the row.

...
- Zombie/unreachable branches checked: direct combo-dish routes/wrappers are stale relative to full edit workflow; merchant-facing combo tag mutation no longer calls admin-only backend routes; category sort branch now invokes its existing SQL; combo delete contract now matches SQL behavior; category delete now blocks active dish references instead of reconciling `dishes.category_id` after unlink.
- Test-proof gaps checked: existing signals cover combo offline rejection at combo-set level, online-combo write-side child dish orderability, public/read/action child orderability, transactional category rename with old-link locking, category sort-only persistence, combo soft-delete association retention, category delete active-reference blocking, category-link rejection in dish create/update transactions, and merchant combo tag selection-only permission alignment. Missing proof remains for retirement/coverage for direct add/remove routes.

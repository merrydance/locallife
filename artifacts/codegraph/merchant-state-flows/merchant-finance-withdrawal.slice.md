...
- Merchant withdrawal creation must create durable local withdrawal intent and a submitted provider command in the same transaction before provider dispatch.
...
22. Withdrawal create validates amount upper bound, requires `Idempotency-Key`, replays same-key/same-request local orders, rejects same-key/different-amount conflicts, generates server-side out-request number, confirms active binding/fee-member id, queries provider balance, persists a `processing` local withdrawal order plus a submitted `external_payment_commands` row in one transaction, does not call provider create in the HTTP request, and returns HTTP `202 Accepted` with `sync_state="unknown"` guidance for new submitted intents.
    Evidence: `locallife/api/baofu_withdrawal.go:296`, `locallife/api/baofu_withdrawal.go:305`, `locallife/api/baofu_withdrawal.go:316`, `locallife/api/baofu_withdrawal_create_result.go:12`, `locallife/api/baofu_withdrawal_create_result.go:18`, `locallife/api/baofu_withdrawal_create_result.go:26`, `locallife/logic/baofu_withdraw_service.go:136`, `locallife/logic/baofu_withdraw_service.go:160`, `locallife/logic/baofu_withdraw_service.go:176`, `locallife/logic/baofu_withdraw_service.go:203`, `locallife/db/sqlc/tx_baofu_withdrawal.go:23`, `locallife/db/sqlc/tx_baofu_withdrawal.go:37`, `locallife/db/sqlc/tx_baofu_withdrawal.go:61`, `locallife/db/query/baofu_withdrawal_order.sql:1`, `locallife/db/query/baofu_withdrawal_order.sql:36`, `locallife/db/migration/000260_add_baofu_withdrawal_idempotency.up.sql:1`, `locallife/db/migration/000260_add_baofu_withdrawal_idempotency.up.sql:16`, `locallife/db/migration/000261_harden_baofu_withdrawal_idempotency_pair.up.sql:1`, `locallife/db/migration/000261_harden_baofu_withdrawal_idempotency_pair.up.sql:16`.

23. Withdrawal command dispatch is an async worker boundary. The recovery scheduler enqueues only submitted Baofu withdrawal commands marked `dispatch_mode="async_worker"`; the worker validates command/order/binding truth, claims the submitted command to `unknown` with a started snapshot before calling provider create, records accepted/rejected/unknown outcome on the same command, mutates the withdrawal order only after an accepted or rejected provider acceptance result, and repairs accepted/rejected command outcomes from the persisted order trace on retry without another provider create call if the command outcome write failed after the order update.
    Evidence: `locallife/db/query/external_payment_fact.sql:114`, `locallife/db/query/external_payment_fact.sql:142`, `locallife/worker/baofu_withdrawal_recovery_scheduler.go:123`, `locallife/worker/baofu_withdrawal_recovery_scheduler.go:132`, `locallife/worker/task_baofu_withdrawal_command_dispatch.go:73`, `locallife/worker/task_baofu_withdrawal_command_dispatch.go:89`, `locallife/worker/task_baofu_withdrawal_command_dispatch.go:125`, `locallife/worker/task_baofu_withdrawal_command_dispatch.go:139`, `locallife/worker/task_baofu_withdrawal_command_dispatch.go:163`, `locallife/worker/task_baofu_withdrawal_command_dispatch.go:181`, `locallife/worker/processor.go:195`, `locallife/worker/processor.go:352`, `locallife/main.go:282`.

24. Withdrawal callback validates parser and payout identity, looks up the withdrawal by provider serial, persists a durable callback fact, and only then enqueues fact application. The callback does not directly mutate the withdrawal row.
...
25. Withdrawal recovery scheduler runs every five minutes, enqueues eligible submitted async-worker commands, scans processing withdrawals older than five minutes, queries provider by out request number, and enqueues the same fact-application task when provider result is terminal.
    Evidence: `locallife/main.go:282`, `locallife/worker/baofu_withdrawal_recovery_scheduler.go:17`, `locallife/worker/baofu_withdrawal_recovery_scheduler.go:61`, `locallife/worker/baofu_withdrawal_recovery_scheduler.go:109`, `locallife/worker/baofu_withdrawal_recovery_scheduler.go:117`, `locallife/worker/baofu_withdrawal_recovery_scheduler.go:123`, `locallife/worker/baofu_withdrawal_recovery_scheduler.go:132`, `locallife/worker/baofu_withdrawal_recovery_scheduler.go:145`, `locallife/worker/baofu_withdrawal_recovery_scheduler.go:151`.

26. Fact-application worker maps upstream state to local `succeeded/failed/returned`, updates only `processing` rows, and treats already-terminal rows as idempotent no-ops.
...
- Fixed 2026-06-12: `external_payment_commands` records withdrawal provider commands with an async-worker dispatch marker for new create intents. The command-dispatch worker claims submitted commands before provider create, then preserves accepted/rejected/unknown provider create diagnostics and sanitized messages on the same command trail; retries can repair accepted/rejected command outcomes from the persisted order trace after an order-update succeeded but command-outcome write failed, without re-dispatching provider create. User-facing create copy remains stable, non-sensitive `202 Accepted` guidance. Withdrawal create idempotency now lives on `baofu_withdrawal_orders`, while command rows remain the provider-call audit trail.
...
- `external_payment_commands`: audit trail for Baofu withdrawal create commands, seeded with a submitted async-worker row at withdrawal creation, claimed to `unknown` before provider dispatch, and later updated with accepted/rejected/unknown provider-create diagnostics for internal investigation.
...
- `locallife/api/baofu_withdrawal_contract_test.go` covers withdrawal balance owner scope, pagination/status text, returned-state merchant-facing guidance, cross-owner detail denial, required idempotency header, same-key replay, conflicting same-key rejection, manager create permission, invalid amount/provider errors, missing fee member, insufficient balance, async `202 Accepted` create guidance, no provider dispatch during API create, and the finance-overview versus Baofu-balance truth-source boundary.
- `locallife/logic/baofu_withdraw_service_test.go` covers provider balance gating, active binding/fee-member checks, idempotent replay/conflict behavior, submitted-command intent creation, no provider dispatch during service create, and merchant-funds owner mapping.
- `locallife/db/sqlc/tx_baofu_withdrawal_test.go` covers atomic withdrawal-order plus submitted-command creation.
...
- `locallife/worker/baofu_withdrawal_recovery_scheduler_test.go`, `task_baofu_withdrawal_command_dispatch_test.go`, and `task_baofu_withdrawal_fact_application_test.go` cover submitted async-command enqueue, provider create dispatch accepted/rejected/unknown outcomes, repair of claimed `unknown` command outcomes from persisted order traces without provider re-dispatch, provider query enqueue, and terminal fact application.
...
- Fixed 2026-06-12: merchant withdrawal create has a real client/server `Idempotency-Key`, commits the withdrawal order plus a submitted async-worker provider command before external dispatch, returns `202 Accepted` for new submitted intents, and no longer calls Baofu provider create inside the HTTP request. Duplicate POSTs for the same unchanged intent replay and conflicting reuse is rejected.
...
- Fixed 2026-06-12: Baofu withdrawal provider create accepted/rejected/unknown outcomes are recorded by the command-dispatch worker in `external_payment_commands`; provider errors keep upstream code and sanitized diagnostic detail while frontend create guidance remains stable and non-sensitive.

...
22. Withdrawal create validates amount upper bound, requires `Idempotency-Key`, replays same-key/same-request local orders, rejects same-key/different-amount conflicts, generates server-side out-request number, confirms active binding/fee-member id, queries provider balance, persists a `processing` local withdrawal order plus a submitted `external_payment_commands` row in one transaction, calls provider create, records provider create outcome on the same command trail, preserves provider-specific create-error code/message diagnostics internally, and returns created/accepted/unknown/rejected semantics.
    Evidence: `locallife/api/baofu_withdrawal.go:294`, `locallife/api/baofu_withdrawal.go:303`, `locallife/api/baofu_withdrawal.go:317`, `locallife/logic/baofu_withdraw_service.go:143`, `locallife/logic/baofu_withdraw_service.go:147`, `locallife/logic/baofu_withdraw_service.go:183`, `locallife/logic/baofu_withdraw_service.go:184`, `locallife/logic/baofu_withdraw_service.go:191`, `locallife/logic/baofu_withdraw_service.go:200`, `locallife/logic/baofu_withdraw_service.go:221`, `locallife/logic/baofu_withdraw_service.go:376`, `locallife/logic/baofu_withdraw_service.go:439`, `locallife/logic/baofu_withdraw_service.go:464`, `locallife/db/sqlc/tx_baofu_withdrawal.go:23`, `locallife/db/sqlc/tx_baofu_withdrawal_test.go:13`, `locallife/db/query/baofu_withdrawal_order.sql:1`, `locallife/db/query/baofu_withdrawal_order.sql:36`, `locallife/db/migration/000260_add_baofu_withdrawal_idempotency.up.sql:1`, `locallife/db/migration/000260_add_baofu_withdrawal_idempotency.up.sql:16`, `locallife/db/migration/000261_harden_baofu_withdrawal_idempotency_pair.up.sql:1`, `locallife/db/migration/000261_harden_baofu_withdrawal_idempotency_pair.up.sql:16`.
...
- `external_payment_commands`: audit trail for Baofu withdrawal create commands, seeded with a submitted row at withdrawal creation and later updated with sanitized provider create-error diagnostics for internal investigation.
...
- Fixed 2026-06-10: merchant withdrawal create has a real client/server `Idempotency-Key` and now commits the withdrawal order plus a submitted provider command before external dispatch; duplicate POSTs for the same unchanged intent replay and conflicting reuse is rejected.

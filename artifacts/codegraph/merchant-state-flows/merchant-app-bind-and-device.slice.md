...
12. Backend stores a normal session row but prefixes the session user agent with `app-bind:` so refresh keeps the long-lived App refresh duration.
    Evidence: `locallife/api/app_bind.go:359`, `locallife/api/app_bind.go:366`, `locallife/api/token.go:71`, `locallife/api/token.go:72`.
...
- Native push dispatcher does not deactivate devices after permanent provider failures. It reports summaries only to the caller. Clarification recorded 2026-06-10: current App native vendor push is real/currently supported, but provider sends can fail while the App is not listed in each vendor app market. If future policy uses "deactivate", that means only the affected `merchant_app_devices` row/device push token, not the merchant account, staff user, or App-wide push capability. Decide whether terminal provider failures should remain report-only, mark that one device degraded, or deactivate that one device after a threshold.
...
- Stale device deactivation or push-dispatch behavior for permanently failed/long-inactive devices. Clarification recorded 2026-06-10: this is the native-push provider terminal-failure policy question, not a bind-code issue; any deactivation would apply only to the affected registered device/push token.

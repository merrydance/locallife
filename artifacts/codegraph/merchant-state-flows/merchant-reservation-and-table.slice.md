...
- Fixed 2026-06-10/2026-06-11: merchant-created phone/walk-in reservations use an explicit offline/phone customer identity model for durable customer traceability; no-show/customer-risk records, customer-side reservation actions, and dining-session billing-group access are not attributed or authorized through the operator staff account.
...
- Fixed 2026-06-11: dining-session billing now follows the same source-aware customer-owner boundary. `OpenDiningSessionTx` can skip default billing-group member creation, `OpenDiningSession` sets that skip for non-online reservation operators, `GetOrCreateDefaultBillingGroup` no longer backfills a default owner/member for the offline reservation creator or legacy `table_reservations.user_id` operator on existing sessions, and billing-group APIs deny merchant actors/legacy operators even if historical default membership exists while still allowing genuinely joined non-merchant members.
...
- Fixed 2026-06-10/2026-06-11: backend tests cover the offline/phone customer identity model, including migration `000264` backfill with contact normalization, blank-name fallback cleanup, index proof, merchant-scoped offline-customer lookup, contact edit identity repointing, API blank-contact rejection, customer detail/action denial for offline operator rows, `/v1/reservations/me` count/list consistency, dish adjustment denial, reservation payment/order/session denial, dining-session default billing member suppression, billing-group API denial for offline operators/merchant actors including legacy missing-`created_by_user_id` rows, joined-member allowance, and no-show behavior attribution that does not punish the operator user id.

...
- Product decision 2026-06-10: merchant-created phone/walk-in reservations need an explicit offline/phone customer identity model; no-show and customer-risk records must not be attributed to the operator staff account.
...
7. Fixed 2026-06-08: customer and merchant reservation create paths both precheck table truth, then lock the table row and revalidate the latest merchant/type/status/capacity state before insert. Customer create also reapplies locked minimum-spend pricing, and both create paths run the final conflict check after the lock. Merchant reservation edit now calls `UpdateReservationTx`, locks the reservation row and target table row when table/date/time/guest count changes, revalidates merchant/type/status/capacity/minimum-spend, and checks the final table/date/time conflict before update.
...
23. Table QR code generation is owner/manager-only and updates `tables.qr_code_url`; changing table number clears the QR URL.
...
24. Flutter merchant App exposes table management from the order-list drawer and routes to `TableGridScreen`.
...
25. Flutter table repository calls the same `/tables/**` backend contract for list, create, patch update, delete, status, tags, images, and QR code.
...
26. Flutter table status actions update the local row from backend response and single-flight duplicate taps per table id.
...
27. Flutter table create/edit sheet writes table metadata first, then binds a selected uploaded image if present, and finally reloads the table list.
...
28. Fixed 2026-06-09: Flutter table image upload now uses `businessType='table'` and `mediaCategory='table'` in both the config sheet and gallery entrypoints, matching backend table-image binding's accepted `media_category='table'` contract.
...
29. Flutter websocket client handles `table_status_change` by patching the in-memory table row.
...
- Product decision 2026-06-10: merchant-created phone/walk-in reservations should use an offline/phone customer identity model. Current runtime uses the operator user id as `user_id`; user reservation list intentionally filters out non-online sources, but downstream user/risk/payment assumptions must remember this is not the real customer account.
- No-show writes a `behavior_decisions` user risk record against reservation user id. For merchant-created phone/walk-in reservations that user id is currently the operator; product decision 2026-06-10 requires a separate offline/phone customer identity so no-show behavior does not punish staff.
...
- Backend tests for the 2026-06-10 offline/phone customer identity model, including no-show behavior attribution that does not punish the operator user id.
...
- Product decision 2026-06-10: merchant-created phone/walk-in reservations should use an offline/phone customer identity model, not staff `user_id` as the customer identity. Current no-show behavior decisions can punish the operator account rather than a real customer.

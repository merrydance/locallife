...
24. Dine-in checkout shows backend payment hints and can submit `use_balance` if the user selects stored-value payment. Its payment-method state now uses backend `payment_assessment.is_balance_payable` plus local member balance, and falls back to WeChat payment when a previously selected balance option becomes backend-disabled. Its page type still includes `reservation`, even though backend membership balance rejects reservation.
    Evidence: `weapp/miniprogram/pages/dine-in/checkout/checkout.ts:38`, `weapp/miniprogram/pages/dine-in/checkout/checkout.ts:83`, `weapp/miniprogram/pages/dine-in/checkout/checkout.ts:249`, `weapp/miniprogram/pages/dine-in/checkout/checkout.ts:254`, `weapp/miniprogram/pages/dine-in/_utils/dine-in-checkout-view.ts:126`, `weapp/miniprogram/pages/dine-in/_utils/dine-in-checkout-view.ts:153`, `weapp/scripts/check-dine-in-checkout-balance-assessment.test.js:74`, `locallife/logic/membership_payment_test.go:36`.

25. Takeout delivery order-confirm maps `payment_assessment.payment_hint` into each cart view, but the direct `takeout` balance-payment path remains intentionally rejected by backend tests.
    Evidence: `weapp/miniprogram/pages/takeout/order-confirm/_utils/takeout-order-confirm-support.ts:416`, `weapp/miniprogram/pages/takeout/order-confirm/index.wxml:157`, `locallife/api/order_test.go:5060`, `locallife/api/order_test.go:5147`.

26. Customer-facing Mini Program `takeaway` checkout is now reachable from restaurant detail through cart and order-confirm. It carries `order_type=takeaway`, previews without address while preserving backend `payment_assessment`, exposes balance only when backend assessment and local balance allow it, omits address/delivery fields for self-pickup, submits `use_balance` when selected, and treats balance-paid orders as terminal only when backend returns `status=paid` and `payment_method=balance`. Adjacent order-type propagation now also protects the takeout home feed from inheriting stale `takeaway` cart-service state and routes takeout/takeaway reorder flows to the matching cart.
    Evidence: `weapp/miniprogram/pages/takeout/restaurant-detail/index.ts:107`, `weapp/miniprogram/pages/takeout/restaurant-detail/index.ts:381`, `weapp/miniprogram/pages/takeout/index.ts:474`, `weapp/miniprogram/pages/takeout/cart/index.ts:42`, `weapp/miniprogram/pages/takeout/cart/index.ts:75`, `weapp/miniprogram/pages/takeout/cart/index.ts:627`, `weapp/miniprogram/pages/orders/detail/index.ts:343`, `weapp/miniprogram/pages/orders/list/index.ts:497`, `weapp/miniprogram/pages/takeout/order-confirm/index.ts:68`, `weapp/miniprogram/pages/takeout/order-confirm/index.ts:180`, `weapp/miniprogram/pages/takeout/order-confirm/index.ts:367`, `weapp/miniprogram/pages/takeout/order-confirm/index.ts:493`, `weapp/miniprogram/pages/takeout/order-confirm/index.ts:556`, `weapp/miniprogram/pages/takeout/order-confirm/_utils/takeout-order-confirm-support.ts:323`, `weapp/miniprogram/pages/takeout/order-confirm/_utils/takeout-order-confirm-support.ts:472`, `weapp/scripts/check-takeaway-balance-checkout.test.js:92`.
...
- `weapp/scripts/check-dine-in-checkout-balance-assessment.test.js` verifies the dine-in checkout payment-method ViewState disables balance and falls back to WeChat payment when backend `payment_assessment.is_balance_payable=false`, even if local member balance is sufficient.
- `weapp/scripts/check-takeaway-balance-checkout.test.js` verifies the customer-facing restaurant-detail/cart/order-confirm `takeaway` path is reachable, does not require address, still previews for backend `payment_assessment`, exposes balance when allowed, sends `use_balance`, omits address/delivery fields for self-pickup, preserves `order_type` through dish/combo/cart/reorder navigation, prevents stale `CartService` order-type inheritance from the takeout home feed, and only treats balance payment as terminal from backend paid status.
- `locallife/api/rules_engine_db_test.go` covers `balance_scene_allowed` parity with the membership scene contract: defaults allow `dine_in/takeaway`, explicit settings allow only configured supported scenes, and legacy `takeout/reservation` scenes are ignored.
- `locallife/logic/membership_settings_test.go` and `locallife/api/membership_test.go` cover owner GET success, full-field owner PUT success, request validation for unsupported scenes and out-of-range max deduction percent, partial PUT preserving omitted existing fields, explicit empty scene arrays staying non-nil empty arrays, and logic missing-row updates using defaults as the merge base.
- `locallife/db/sqlc/membership_settings_migration_test.go` covers isolated migration proof for clean DBs through `000253`, dirty historical `251 -> 252` cleanup of `takeout`/`reservation`/null scenes before constraints, and weak already-applied `252 -> 253` hardening without relying on same-number migration replay.

Missing high-value tests:

- None currently known for the `merchant-membership-settings` defects fixed in this branch; future changes should add scenario proof for any new supported stored-value order type before exposing it.
...
- Backend direct order and transaction code now has API happy-path proof for `takeaway` with `use_balance=true`, promotion preview tests cover `takeaway` membership scenes, and Mini Program user-reachable `takeaway` balance checkout is covered by `check-takeaway-balance-checkout`.
...
- Fixed 2026-06-07: Dine-in checkout payment-method UI now disables stored-value payment from backend `payment_assessment.is_balance_payable` as well as local member balance, and clears a stale balance selection before order creation when the backend says balance is unusable due to merchant settings or stacking.
...
- Failure/retry branches checked: duplicate save guard, last-write-wins concurrent PUT, omitted fields preserving existing/default merge-base values, explicit empty scene arrays, historical frontend `takeout/reservation` values, DB rows with unsupported/null scenes cleaned or sanitized, preview/direct-order scene mismatch, and the fixed dine-in UI guard that prevents choosing balance when backend payment assessment rejects it.
- Reader/consumer branches checked: merchant settings page, cart/order preview, dine-in checkout payment selection, customer-facing `takeaway` cart/order-confirm payment selection, direct order creation, promotion engine `balance_scene_allowed`, membership payment validation, and customer-facing payment assessment.
...
- Test-proof gaps checked: existing tests cover manager denial, owner GET/PUT API contract, request validation, direct balance validation, dine-in and `takeaway` order balance, scene disallow, takeout/reservation rejection, promotion engine takeaway behavior, payment assessment shape, rules-engine `balance_scene_allowed` parity, clean/dirty/weak-schema migration convergence through `000252/000253`, partial PUT merge semantics including explicit empty arrays and missing-row default merge base, the frontend/backend/migration scene contract script, dine-in checkout balance-option alignment with backend payment assessment, and Mini Program `takeaway` checkout reachability plus per-order-type preview/direct consistency.

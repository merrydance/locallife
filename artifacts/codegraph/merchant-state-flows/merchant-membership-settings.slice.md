...
- Test-proof gaps checked: existing tests cover manager denial, owner GET/PUT API contract, request validation, direct balance validation, dine-in and `takeaway` order balance, scene disallow, takeout/reservation rejection, promotion engine takeaway behavior, payment assessment shape, rules-engine `balance_scene_allowed` parity, local migration convergence through `000253`, partial PUT merge semantics including explicit empty arrays and missing-row default merge base, the frontend/backend/migration scene contract script, and dine-in checkout balance-option alignment with backend payment assessment. Missing proof remains for clean/dirty scratch migration fixtures, Mini Program `takeaway` checkout reachability, and per-order-type preview/direct consistency.

...
Status: partially fixed; Mini Program voice-control removal/backend no-op compatibility, print-disabled auto-accept normalization, Mini Program cloud-printer reconciliation recovery UI, Flutter App backend-truth auto-accept convergence, Flutter BLE accepted-receipt backend config/event boundaries, and Flutter saved-printer reconnect/disconnect/failure copy fixed through 2026-06-12
Risk class: G2/G3 - merchant configuration controls automatic order acceptance, print task dispatch, cloud-printer provider state, async recovery surfaces, and payment-domain outbox retry side effects
...
25. Payment-domain outbox consumes `auto_accept_paid_orders`. It only auto-accepts paid orders when display config enables auto-accept, printing, order type, accepted trigger, and at least one eligible active Feieyun printer. Fixed 2026-06-12: accepted print scheduling uses a stable `order:{id}:accepted` task key, an asynq uniqueness window, duplicate-task no-op handling, and an order-level `print_logs` task-key check before retry scheduling so payment outbox retries do not enqueue another accepted print after one is already queued or logged.
    Evidence: `locallife/worker/task_payment_domain_outbox.go:120`, `locallife/worker/task_payment_domain_outbox.go:169`, `locallife/worker/task_payment_domain_outbox.go:184`, `locallife/worker/task_payment_domain_outbox.go:199`, `locallife/worker/task_payment_domain_outbox.go:203`, `locallife/worker/task_payment_domain_outbox.go:211`, `locallife/worker/task_payment_domain_outbox.go:224`, `locallife/worker/task_payment_domain_outbox.go:241`, `locallife/worker/task_payment_domain_outbox.go:260`, `locallife/worker/task_payment_domain_outbox.go:272`, `locallife/worker/task_payment_domain_outbox.go:278`, `locallife/worker/task_payment_domain_outbox.go:279`, `locallife/worker/task_payment_domain_outbox.go:294`.
...
- Fixed 2026-06-12: payment outbox accepted-print retries now keep one stable accepted print task boundary through queue uniqueness, duplicate enqueue no-op handling, and existing `print_logs.task_key` suppression.
...
- Payment outbox auto-accept calls conditional merchant order logic, so repeated execution after status changes will no-op or skip through state checks. Fixed 2026-06-12: accepted-print side effects use the same stable task key across retries, pass an asynq uniqueness option, treat duplicate enqueue as already scheduled, and skip retry scheduling when the order already has a matching accepted print log.
...
- Auto-accept runs asynchronously in payment-domain outbox processing after paid order facts. Fixed 2026-06-12: accepted print retry convergence is guarded at both the queue boundary and the durable print-log task-key boundary.
...
- Flutter BLE connect and saved-device reconnect failures remain local App state and are not visible in backend device/printer recovery pages. Fixed 2026-06-12: accepted-receipt print attempts and outcomes are backend-visible through `merchant_local_print_events`; saved-device reconnect, disconnect state clearing, and stable scan/connect/print-failure copy are now covered in App tests.
...
- `locallife/worker/task_payment_domain_outbox_test.go` covers auto-accept after paid order under display/printer config, accepted print queue uniqueness, retry recovery when no print log exists, skipping retry enqueue when a matching accepted `print_logs.task_key` exists, and treating asynq duplicate-task responses as already scheduled.
...
- `merchant_app/test/printer_provider_test.dart` covers saved Bluetooth printer reconnect, auto-reconnect failure copy, unexpected disconnect clearing connected-device state, stable manual-connect failure copy, stable scan-failure copy, and stable receipt-print failure copy.
...
- Fixed 2026-06-12: worker tests prove a paid order with `auto_accept_paid_orders=true` updates order state and schedules a single stable accepted print task boundary across outbox retries: queue duplicates are no-op, a missing print log still allows recovery enqueue, and an existing accepted `print_logs.task_key` suppresses another enqueue.
...
- Fixed 2026-06-12: Flutter local-printer tests now cover duplicate receipt suppression through the shared accept coordinator, backend display-config enforcement for accepted BLE receipts, started/success/failed local print event reporting, saved-device reconnect, disconnected-device failure copy, and stable scan/connect/print-failure copy. Remaining local-printer proof gap is real Android BLE hardware behavior.
...
- Fixed 2026-06-12: Flutter local Bluetooth accepted receipts honor backend display config and add backend-visible local-print observability/deduplication through `merchant_local_print_events`. Fixed 2026-06-12: payment outbox accepted-print retries now have queue/log dedupe proof. Fixed 2026-06-12: saved-device reconnect, disconnected-device failure copy, and stable scan/connect/print-failure copy are covered through the App printer provider test boundary. Remaining follow-up is real-device BLE validation; BLE receipts remain outside cloud-printer `print_logs`.

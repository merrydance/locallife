...
Page({
    ...
    onCompanyNameInput(e) { this.updateFormData('companyName', e.detail.value); },
    onContactNameInput(e) { this.updateFormData('contactName', e.detail.value); },
    ...
    onSubmit() {
        ...
        if (!accountPermitImages.length)
            return wx.showToast({ title: '请上传开户许可证', icon: 'none' });
        if (!idCardFrontImages.length)
            return wx.showToast({ title: '请上传法人身份证正面', icon: 'none' });
        if (!idCardBackImages.length)
            return wx.showToast({ title: '请上传法人身份证反面', icon: 'none' });
        ...
        if (!formData.address)
            return wx.showToast({ title: '请选择公司地址', icon: 'none' });
        if (!formData.creditCode)
            return wx.showToast({ title: '缺少营业执照信息', icon: 'none' });

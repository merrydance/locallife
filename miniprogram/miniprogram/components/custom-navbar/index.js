...
Component({
    ...
    methods: {
        initNavBar() {
            ...
            // 同步到全局状态
            global_store_1.globalStore.set('navBarHeight', navBarHeight);
            // 通知父页面
        ...
        subscribeToLocationChanges() {
            this._unsubscribe = global_store_1.globalStore.subscribe('location', (newLocation) => {
        ...
        updateLocationDisplay() {
            ...
            if (this.properties.location) {
                ...
                // 否则使用全局location
                const location = global_store_1.globalStore.get('location');
            ...
            wx.getLocation({
                ...
                success: (res) => __awaiter(this, void 0, void 0, function* () {
                    try {
                        ...
                        global_store_1.globalStore.updateLocation(res.latitude, res.longitude, name, locationInfo.address);
                        // 局部更新显示
                        ...
                        global_store_1.globalStore.updateLocation(res.latitude, res.longitude, displayLocation, displayLocation);
            ...
            wx.chooseLocation({
                success: (res) => __awaiter(this, void 0, void 0, function* () {
                    try {
                        ...
                        // 使用GlobalStore统一管理
                        global_store_1.globalStore.updateLocation(res.latitude, res.longitude, name, res.address || locationInfo.address);
                        // 局部更新显示
                        this.setData({ displayLocation: name });
                        // 通知父页面
                        this.triggerEvent('locationchange', {
                        ...
                        global_store_1.globalStore.updateLocation(res.latitude, res.longitude, name, res.address);
                        this.setData({ displayLocation: name });
                        this.triggerEvent('locationchange', res);
                    }
                }),

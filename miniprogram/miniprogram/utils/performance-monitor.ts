...
import { imageLazyLoader } from './image-lazy-load'
...
class PerformanceMonitor {
  ...
  private init(): void {
    try {
      ...
      if (this.enabled) {
        logger.info('性能监控已启用', undefined, 'PerformanceMonitor')
        this.startMonitoring()
      }
    } catch (e) {
      logger.warn('无法初始化性能监控', e, 'PerformanceMonitor')
    }
  }
  ...
  private startMonitoring(): void {
    ...
    this.updateInterval = setInterval(() => {
      this.collectMetrics()
    }, this.UPDATE_FREQUENCY) as unknown as number
  }
  ...
  private collectMetrics(): PerformanceMetrics {
    ...
    // 内存信息
    try {
      const performance = wx.getPerformance() as unknown as WxPerformance
    ...
    // 图片统计
    try {
      const imageStats = imageLazyLoader.getStats()
   ...
   * 记录网络请求
   */
  recordRequest(fromCache: boolean = false): void {
    if (!this.enabled) return
...
if (typeof global !== 'undefined') {
  global.perfMonitor = performanceMonitor
}

...
import { imageLazyLoader, PLACEHOLDER_IMAGE } from './image-lazy-load'
 ...
 * 获取带错误占位符的图片URL
 */
export function getImageWithFallback(url: string | undefined | null, size?: number): string {
 ...
 * 批量格式化图片URL（用于列表）
 */
export function formatImageUrls(urls: string[], size?: number): string[] {
...
export function preloadImages(urls: string[], priority: boolean = false): void {
  ...
  imageLazyLoader.preload(formattedUrls, priority)
}

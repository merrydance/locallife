...
在系统尚未生产上线的前提下，完成直接切主判前的最后验收，而不是再设计 shadow run 或灰度切换。

### 必做项

1. 复核当前正式主判对三类 claim_type 的结果覆盖率。
2. 复核平台兜底率、误追责风险、误伤风险、性能指标。
3. 整理直接切主判所需的回退开关、监控点、回归清单。

### 输出

1. 有一套可执行的预上线验收记录，而不是只停留在设计预期。

### 退出条件

1. 三类 claim_type 均可走通正式主判与动作链。
2. 平台兜底率、误追责风险、误伤风险处于可接受范围。
3. 延迟与失败回退路径符合上线要求。
4. 直接切主判所需的监控与回退手段已准备完成。

## 5.7 Phase 6 直接切主判

### 目标

在系统尚未生产上线的前提下，直接让新主判接管正式主路径，不再额外设计灰度阶段。
...
5. 最后做 Phase 5 和 Phase 6，完成预上线验收与直接切主判。
...
3. 当前主线已经进入 Phase 4，因此最应该补的是 Phase 4 delivery map。
4. 由于系统尚未生产上线且确认直接全量切换，Phase 5 也应补一份“预上线验收 + 直接切主判”的 delivery map。

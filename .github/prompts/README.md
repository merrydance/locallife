...
- Payment-domain prompts should mirror the matching provider domain README such as `.github/standards/domains/wechat-payment/README.md` or `.github/standards/domains/baofu-payment/README.md`: official docs first, then capability-group identification, then caller propagation and focused validation.
...
8. "审查这个 db/query 和 migration 变更，重点看 sqlc 传播、索引遗漏和事务风险。"
Expected target: `backend-sql-review.prompt.md`

9. "给这个小程序页面做页面方案，先诊断 setData 热点和弱网体验，再给实施方案。"
Expected target: `weapp-implementation.prompt.md`

10. "修一下小程序支付完成后返回页状态丢失和重复点击支付的问题。"
Expected target: `weapp-implementation.prompt.md`

11. "改一下小程序页面的列表空态和错误态。"
Expected target: `weapp-implementation.prompt.md`

12. "把这个小程序页面完全用 TDesign 重构，整页重新布局成极简风格，复杂区块拆成组件，新增删除改成图标按钮。"
Expected target: `weapp-implementation.prompt.md`

13. "从整体升级角度审查一下 weapp 的交互和风格，既看现行规范，也看后端真相、页面连贯性和常见低质量模式。"
Expected target: `weapp-review.prompt.md`

14. "给 merchant_app 做绑定码登录、推送注册和前台服务的实现请求模板。"
Expected target: `flutter-implementation.prompt.md`

15. "修这个 Flutter 商户端漏单问题，先追真实根因，再说明为什么不能只在 Widget 打补丁。"
Expected target: `flutter-bugfix.prompt.md`

16. "审查这个 Flutter 商户端改动，重点看 provider wiring、消息去重、弱网恢复和接单链路。"
Expected target: `flutter-review.prompt.md`

17. "这个需求要同时改 backend 和 web，帮我整理一份实现请求。"
Expected target: `general-implementation.prompt.md`

18. "把这段报销审批流程整理成 Mermaid，补上驳回和超时分支。"
Expected target: `business-flow-mermaid.prompt.md`

19. "把这组任务按 开发 -> review -> 修复 -> review -> 文档同步 的顺序跑完，直到任务清单完成。"
Expected target: `general-task-loop.prompt.md`

20. "把这次线上事故的结论落成规则、workflow、测试和 runbook 更新清单。"

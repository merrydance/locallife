...
本文件中的自定义视觉语言与品牌 token 默认服务于顾客侧页面。商户、运营、平台、骑手等非顾客侧页面默认不继承顾客侧品牌色、装饰性风格或定制视觉 token，除非标准中另有明确批准。
...
- 非顾客侧页面应优先体现工具性、清晰度、稳定层级和长期维护一致性，默认使用 TDesign Miniprogram 的视觉语言作为主干。
- 非顾客侧页面可以复用统一的间距、安全区、page shell、触控热区和基础布局节奏，但不要求和顾客侧保持同一套颜色或装饰风格。

...
- **页面次操作**: 底部成组次操作统一使用 `theme="default"`，尺寸与主操作保持一致；页面内联辅助操作可更轻，但不能让用户分不清谁是主操作。
...
- 这类动作默认使用 `size="small"`，并通过 `theme`、`shape`、`icon` 控制轻量层级；不要用大号文字按钮把局部动作做成和页面主提交同级的视觉重量。
...
- 非空且以列表运维为主的页面，优先使用右下角悬浮新增按钮。
- 空状态可以使用与说明文案配套的显式主按钮；只要动作语义一致，不强求和列表态入口完全同形。
- 若首屏需要先解释“为什么为空”或“新增后会发生什么”，应优先保证说明与主按钮的可发现性，而不是机械坚持悬浮入口。

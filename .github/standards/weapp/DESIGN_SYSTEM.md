...
组件约束：优先直接使用 TDesign 原生按钮能力；允许做官方支持的主题、间距、安全区和页面布局适配，不允许无必要地修改 TDesign 内部状态行为、图标语义或默认交互反馈。
...
- 对用户可见控件，默认禁止先写原生 `input`、`textarea`、`picker`、`switch`、`checkbox-group`、`radio-group`、`button` 等再事后解释；只有在确认 TDesign 当前版本没有合适能力时，才允许退回原生控件。
- 允许做主题色、间距、安全区和页面组合层适配，不允许无必要地改写组件内部状态语义。
- 不允许为了局部页面审美去覆盖 TDesign 内部类名、节点结构、伪元素、交互动效或状态色；默认只允许在官方文档明确支持的 theme、props、外层布局、容器间距、安全区和组合方式上做适配。
- 只有在官方文档明确允许、且已确认存在平台限制、组件缺陷或缺失能力时，才允许做最小范围的 TDesign 样式扩展，并且必须优先说明“为什么官方支持方式和外层组合无法解决”。
- 原生组件例外主要限于三类：基础结构原语、平台能力桥接组件、以及 TDesign 当前版本确实缺失的控件能力。除此之外，把原生控件混入 TDesign 页面默认视为风格与交互漂移。

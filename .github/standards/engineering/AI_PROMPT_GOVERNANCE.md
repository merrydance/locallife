...
### 3.5.1 User-Reported Defect Closure Gate

当任务来自用户报告的具体 bug、线上回归或“结果不对/搜不到/列表为空/筛选不生效”一类体验问题时，implementation 与 review prompt 必须把原始失败场景闭环作为验收门槛，而不是只验证改动点本身：

- 先把用户原话中的失败行为改写成一个可执行验收用例，说明输入、上下文、预期结果和失败表现。
- 如果用户提到已知正常路径、参考页面或“另一个入口可以工作”，必须对照该路径的事件、请求参数、service/API、adapter/view model、ViewState 与渲染分支，而不是只借用结论。
- 搜索、列表、筛选、分页、tab 和空态类 bug 必须追完整链路：输入或筛选状态 -> 请求参数 -> API 组合或并发源 -> response normalization -> tab / list / empty / error rendering。
- 多个异步源组合时，必须验证部分失败行为；一个源失败不得隐藏另一个源的有效结果，服务失败也不得在没有证据时伪装成真实空态。
- 回归验证应优先覆盖用户的原始失败场景，且在可行时做到修复前失败、修复后通过；如果只能做近似自动化或手工验证，必须说明覆盖差距和剩余风险。
- 交付或 review closeout 必须回答：真实根因是什么、为什么原实现或前一次修复会漏掉、哪条测试或验证证明了用户原始场景。

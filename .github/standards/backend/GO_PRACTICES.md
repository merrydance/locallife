...
- Redis、WebSocket 推送、本地缓存和内存投影只能做性能或体验优化，不能作为订单、支付、退款、代取、预订、库存等主状态真值来源。
...
- 订单、支付、退款、代取、预订、库存等高风险逻辑新增分支时，除了 happy path，至少补一个 conflict / duplicate / stale-state / retry 方向的断言说明。

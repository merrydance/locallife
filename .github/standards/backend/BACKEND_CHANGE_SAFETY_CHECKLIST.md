...
- 如果改动接入、解析、映射或暴露外部 API / provider 数据，是否已按 `.github/standards/backend/EXTERNAL_API_CONTRACT_STANDARDS.md` 确认官方文档、样例、字段矩阵、错误码、枚举、单位和条件必填规则？
- 新增或修改 provider 字段时，是否同步更新了 domain README、source matrix、field matrix、fixture、DTO、parser、validator、error mapper 和调用方？
- 是否存在盲猜字段、从旧 DTO / 相邻接口类比、未知枚举成功、缺字段成功、空 DTO 成功、静默降级或 raw provider error 透传？
- Provider 异常、超时、解析失败、签名失败和未知状态是否到达一个安全的结构化日志边界，并给调用方或前端稳定业务语义和下一步指引？

## 4. Cross-Layer Completeness

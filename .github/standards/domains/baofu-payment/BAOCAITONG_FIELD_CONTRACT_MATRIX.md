...
| `withdraw.request.body.feeMemberId` | `feeMemberId` | String | 32 | C | conditional rule in description; length=32 | 用户自己承担手续费必传，与客户号contractNo一致需用户承担手续费时要提前和商务申请配置 | Current LocalLife withdrawal sends the withdrawing account binding `sharing_mer_id` as the fee-bearing customer id per Baofoo support confirmation on 2026-05-26; preserve official length/type at boundary |

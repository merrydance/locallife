...
- [x] 按宝付测试环境口径省略 `subMchId`，用本小程序下真实 `sub_openid` 发起一次 `unified_order` 请求形态验证；sandbox 已返回 `WAIT_PAYING` 和可解析 `wc_pay_data`，但宝付已确认测试环境不支持真实下单，因此仍不能作为支付回调或后续分账链路证明。生产环境仍必须上送聚合商户报备返回的 `subMchId`。
...
2026-05-05 宝付技术支持回复：测试环境统一下单不要上送 `subMchId`，生产环境需要上送；测试环境不支持真实下单；`bind_sub_config` 返回 `SUCCESS` 表示绑定成功但可能需要约 30 分钟才能发起交易；测试环境商户/渠道资料为虚拟资料，不会真实发往渠道。因此此前携带 `subMchId=4000***0573` 的 sandbox `PAY_CHANNEL_NOT_SUPPORT` 不能再作为生产契约字段漂移证据；下一次 sandbox 统一下单的单变量重试必须使用仓库内 `go run ./cmd/baofu_unified_order_smoke`，确认 effective wire `subMchId=omitted_by_client`，且验收目标是不含 `subMchId` 的请求形态、业务 `dataContent` 解析和安全错误分类；即使 sandbox 返回 `wc_pay_data`，也只证明 payload 可解析，不证明真实支付/回调/分账链路。生产 readiness 和发包仍以商户报备 `subMchId` 为必填。
...
| 2026-05-05 | sandbox | `https://mch-juhe.baofoo.com/api` | `BAOFU_UO_20260505113416` | requested `4000***0573`; effective wire `omitted_by_client` | 100 | no | no | no | `77662c11` | Repo smoke proved sandbox client omits `subMchId` on the wire. Baofoo returned a business payload shape where `chlRetParam.order_id` is numeric; local DTO still required string and the unmarshal failure was reported as upstream_code `SUCCESS`. Fixed by accepting numeric/string `order_id` and classifying business payload unmarshal failures as `INVALID_DATA_CONTENT`, not `SUCCESS`. |
| 2026-05-05 | sandbox | `https://mch-juhe.baofoo.com/api` | `BAOFU_UO_20260505114340` | requested `4000***0573`; effective wire `omitted_by_client` | 100 | yes | no | no | `77662c11` | Positive unified-order sandbox parsing evidence after numeric `order_id` fix. Response parsed as `txnState=WAIT_PAYING`, `resultCode=SUCCESS`, masked `tradeNo=2605***1948`, and `wc_pay_data=true`. Because Baofoo confirmed sandbox does not support real payment, this is request/response contract evidence only; payment callback and downstream分账 still require production first-order or Baofoo real-transaction environment. |
| 2026-05-05 | sandbox | `https://mch-juhe.baofoo.com/api` | `BAOFU_UO_20260505115158` | requested `4000***0573`; effective wire `omitted_by_client` | 100 | yes | no | no | `d1108394` | Repeat positive unified-order sandbox parsing evidence with the updated smoke note. Response parsed as `txnState=WAIT_PAYING`, `resultCode=SUCCESS`, masked `tradeNo=2605***1952`, and `wc_pay_data=true`; still not real payment/callback evidence under Baofoo sandbox limits. |
...
| 2026-05-05 | sandbox | `https://mch-juhe.baofoo.com/api` | `BAOFU_UO_20260505115158` / `2605***1952` | success; `txnState=SUCCESS`, normalized `success`, `resultCode=SUCCESS`, `wc_pay_data=false` | no | no | `9d36f160` | Positive order-query sandbox evidence for an order created by `unified_order` smoke. Query by `outTradeNo` returned the same masked `tradeNo`; this proves query response parsing and terminal-state normalization. Baofoo sandbox still does not prove real payment callback or downstream settlement. |
| 2026-05-05 | sandbox | `https://mch-juhe.baofoo.com/api` | `BAOFU_UO_20260505114340` / `2605***1948` | success; `txnState=SUCCESS`, normalized `success`, `resultCode=SUCCESS`, `wc_pay_data=false` | no | no | `9d36f160` | Repeat positive order-query sandbox evidence for a prior unified-order smoke order. Confirms `order_query` can recover final state by `outTradeNo`; no callback/local fact was exercised in this command. |
...
| 2026-05-05 | sandbox | `https://llapi.merrydance.cn/v1/webhooks/baofu/payment` | unknown sandbox order callback | unknown | parse failed on JSON scalar type mismatch for documented string field `merId`; HTTP 401 | no | no | no | next fix | PEM/public-key configuration was already past the previous failure. Baofoo support confirmed the contract type is string; local parser keeps the project contract as string while accepting JSON string/number defensively at the inbound callback boundary and normalizing to string before required-field, enum, and signature-protected fact parsing. |
| 2026-05-05 | sandbox | `https://llapi.merrydance.cn/v1/webhooks/baofu/payment` | `BAOFU_UO_20260505161741` | `2605***1965` | parsed callback and reached local order lookup; no local `payment_orders` row; HTTP 500 | no | no | no | next fix | This is positive parser/verification progress: callback exposed `outTradeNo` and Baofoo `tradeNo` to the handler. It cannot be ACKed because the order was created by the standalone smoke path and was not persisted through LocalLife `/v1/payments`; next callback fact test must use a locally created payment order or a controlled local fixture. |

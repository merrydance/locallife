...
This document is a working map for implementing the LocalLife Baofoo BaoCaiTong contract layer. It does not replace the official Baofoo docs. The field-by-field build checklist lives in `BAOCAITONG_FIELD_CONTRACT_MATRIX.md`; the full row-level audit and current local coverage live in `API_CONTRACT_COVERAGE_AUDIT.md`; the source page ledger lives in `CONTRACT_SOURCE_MATRIX.md`.

## 1. Source Freshness

- Live source: authenticated Baofoo doc.mandao BCT documentation pages, refreshed on 2026-05-08 with the supplied documentation account. Do not store login URLs with access tokens in repo docs.
- Cleaned source output: `.github/standards/domains/baofu-payment/official-sources/doc-mandao/` contains 107 cleaned Markdown article extracts from the 117-page authenticated catalog, plus `INDEX.md` and `SHA256SUMS`.
- Comparison: the 66 previously committed article captures had identical raw source hashes in the 2026-05-08 refresh. The refresh added the remaining article pages from the authenticated catalog, including `queryCard`, `bct-1f9o5s1lqlean`, and `bct-1f9qrdjnaqra5`.
- Empty article pages in the catalog are navigation/container pages, not field-truth pages: `APIList`, `payFunc`, `bct-1f9qh72cabkr8`, `bct-1f9qm38du1agl`, `bct-1f9qm3fvoqleu`, `bct-1f9qlnv7p9ucd`, `bct-1fao0hurn0adk`, `bct-1f9o61rnb442i`, `bct-1fbc7mbon00ct`, `bct-1fdiuecpe5e4e`.
...
| Field audit | `API_CONTRACT_COVERAGE_AUDIT.md` |
| Source matrix | `CONTRACT_SOURCE_MATRIX.md` |
| Sandbox evidence | `SANDBOX_EVIDENCE_LEDGER.md` |

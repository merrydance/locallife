...
- Do not let Baofoo DTOs, raw upstream payloads, signatures, certificate material, full identifiers, bank cards, identity numbers, or unsafe provider error text leak into logic, API responses, frontend state, or unsafe logs. Do not discard provider failure codes or reasons: preserve them in sanitized internal diagnostics, and let user-actionable provider failure reasons reach frontend semantics only after `locallife/baofu` classification and sanitization. Classified LocalLife guidance is only the user-facing fallback for unsafe provider text; raw payloads, internal IDs, credentials, and unstable diagnostics must remain internal.

...
### P1-3 商家打印机页：隐藏日常任务里的技术字段，重排操作入口
...
- 当前状态：计划已制定，尚未实施业务代码。
- 下一步：用户确认后按推荐实施顺序执行；每完成一个任务运行对应验证并单独提交。

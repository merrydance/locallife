      ...
      <PageContent>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {marketingTools.map((tool) => (
            <Link 
              key={tool.title} 
              href={tool.availability === "已上线" ? tool.href : "#"}
              className={tool.availability === "已上线" ? "cursor-pointer" : "cursor-not-allowed opacity-75"}
            >
              <Card className="h-full hover:shadow-md transition-all border-slate-100 group overflow-hidden">
                <CardContent className="p-6 flex flex-col h-full">
                  ...
                  <h3 className="font-bold text-lg mb-2 flex items-center group-hover:text-primary transition-colors">
                    {tool.title}
                    {tool.availability === "已上线" && <ChevronRight className="h-4 w-4 ml-1 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all" />}
                  </h3>
